...
function sanitizeNumberString(numberString) {
  return sanitizeExponentialNumberString(numberString, (nonExpoNumString) => {
    ...
    return isValidNumber(sanitizedValue) ? isNegativeDecimalOnlyZeros.test(sanitizedValue) ? sanitizedValue : Number(sanitizedValue).toString() : nonExpoNumString;
  });
}
...
var allDecimalsExceptLast = new RegExp(`[.](?=.*[.])`, "g");
var everythingExceptNumbersDecimalsAndMinusSigns = new RegExp("[^0-9-.]", "g");
var defaultGroupSeparator = new RegExp(",", "g");
...
function delocalizeNumberString(numberString, locale) {
  return sanitizeExponentialNumberString(numberString, (nonExpoNumString) => {
    const delocalizedNumberString = nonExpoNumString.replace(getMinusSign(locale), "-").replace(getGroupSeparator(locale), "").replace(getDecimalSeparator(locale), ".").replace(allDecimalsExceptLast, "").replace(everythingExceptNumbersDecimalsAndMinusSigns, "");
  ...
  return sanitizeExponentialNumberString(numberString, (nonExpoNumString) => {
    if (nonExpoNumString) {
      ...
      if (!isNaN(number)) {
        ...
        const localizedNumberString = parts.map(({ type, value }) => {
          switch (type) {
            case "group":
...
var Input = proxyCustomElement(class extends H {
  constructor() {
    ...
    this.setValue = ({ committing = false, nativeEvent, origin, previousValue, value }) => {
      const previousLocalizedValue = this.type === "number" ? localizeNumberString(this.previousValue, this.locale, this.groupSeparator) : "";
      const sanitizedValue = this.type === "number" ? sanitizeNumberString(value) : value;
      const newValue = this.type === "number" && value && !sanitizedValue ? isValidNumber(this.previousValue) ? this.previousValue : "" : sanitizedValue;
      const newLocalizedValue = this.type === "number" ? localizeNumberString(newValue, this.locale, this.groupSeparator) : "";
  ...
  render() {
    ...
    const inputClearButton = h("button", { "aria-label": this.intlClear || TEXT2.clear, class: CSS.clearButton, disabled: this.disabled || this.readOnly, onClick: this.clearInputValue, tabIndex: this.disabled ? -1 : 0, type: "button" }, h("calcite-icon", { icon: "x", scale: "s" }));
...
@esri/calcite-components/dist/components/number.js:
  (*!
...
@esri/calcite-components/dist/components/calcite-input.js:
   ...
   *)
*/
//# sourceMappingURL=calcite-input-3RE5W34E.js.map
