import "./chunk-AC2VUBZ6.js";

// node_modules/@arcgis/core/chunks/vxlLayer.js
function e(e2, r2) {
  for (var t2 = 0; t2 < r2.length; t2++) {
    const n2 = r2[t2];
    if ("string" != typeof n2 && !Array.isArray(n2)) {
      for (const r3 in n2)
        if ("default" !== r3 && !(r3 in e2)) {
          const t3 = Object.getOwnPropertyDescriptor(n2, r3);
          t3 && Object.defineProperty(e2, r3, t3.get ? t3 : { enumerable: true, get: () => n2[r3] });
        }
    }
  }
  return Object.freeze(Object.defineProperty(e2, Symbol.toStringTag, { value: "Module" }));
}
var r;
var t;
var n;
var o = { exports: {} };
r = o, t = "undefined" != typeof document && document.currentScript ? document.currentScript.src : void 0, n = function(e2) {
  var r2, n2, o2 = void 0 !== (e2 = e2 || {}) ? e2 : {};
  o2.ready = new Promise(function(e3, t2) {
    r2 = e3, n2 = t2;
  }), o2.expectedDataFileDownloads || (o2.expectedDataFileDownloads = 0), o2.expectedDataFileDownloads++, function(e3) {
    function r3() {
      var e4 = "";
      o2.FS_createDataFile("/", "common_defines.glsl", uo(e4), true, true, false);
      var r4 = "";
      o2.FS_createDataFile("/", "voxel_apron_common.glsl", uo(r4), true, true, false);
      var t2 = "";
      o2.FS_createDataFile("/", "voxel_apron_from_staging_cs.glsl", uo(t2), true, true, false);
      var n3 = "ayxUgPAuy41w60egGGGQH1wVrEgrmkhCZ0qqSXetlTfyexv7Zbu7+jDlX5BiAAIofrbsBmXQ2EXPAb8d5jAo/WpFOw==";
      o2.FS_createDataFile("/", "voxel_apron_to_staging_cs.glsl", uo(n3), true, true, false);
      var a3 = "";
      o2.FS_createDataFile("/", "voxel_common.glsl", uo(a3), true, true, false);
      var i2 = "";
      o2.FS_createDataFile("/", "voxel_dbg_ray_ps.glsl", uo(i2), true, true, false);
      var s2 = "";
      o2.FS_createDataFile("/", "voxel_dda.glsl", uo(s2), true, true, false);
      var u2 = "";
      o2.FS_createDataFile("/", "voxel_debug_mesh_common.glsl", uo(u2), true, true, false);
      var l2 = "";
      o2.FS_createDataFile("/", "voxel_debug_mesh_ps.glsl", uo(l2), true, true, false);
      var c2 = "";
      o2.FS_createDataFile("/", "voxel_debug_mesh_vs.glsl", uo(c2), true, true, false);
      var f2 = "ayxUgPAuy41g4UinBH+HF0EBgXMF+QVIZVyiWTPqw3r8cQ/BbZ206y/jUpB3Fw4xSKr2FF3JxQwvJEKKu02k";
      o2.FS_createDataFile("/", "voxel_dynamic_sections_depth_ps.glsl", uo(f2), true, true, false);
      var d2 = "";
      o2.FS_createDataFile("/", "voxel_dynamic_sections_ps.glsl", uo(d2), true, true, false);
      var E2 = "ayxUgPAuy418/UKsHXWSBEtfhlpP/ghHfE2jHDG+2m3heDz6cIq75yvpbrxhERksTqvrOHLKmEUkO13ElEo=";
      o2.FS_createDataFile("/", "voxel_dynamic_sections_query_ps.glsl", uo(E2), true, true, false);
      var g2 = "";
      o2.FS_createDataFile("/", "voxel_dynamic_sections_vs.glsl", uo(g2), true, true, false);
      var p2 = "";
      o2.FS_createDataFile("/", "voxel_editable_slice_plane_gs.glsl", uo(p2), true, true, false);
      var h2 = "";
      o2.FS_createDataFile("/", "voxel_editable_slice_plane_ps.glsl", uo(h2), true, true, false);
      var m2 = "";
      o2.FS_createDataFile("/", "voxel_editable_slice_plane_vs.glsl", uo(m2), true, true, false);
      var v2 = "";
      o2.FS_createDataFile("/", "voxel_missing_bricks_stage1_cs.glsl", uo(v2), true, true, false);
      var w2 = "";
      o2.FS_createDataFile("/", "voxel_missing_bricks_stage2_cs.glsl", uo(w2), true, true, false);
      var y2 = "";
      o2.FS_createDataFile("/", "voxel_missing_bricks_stage3_cs.glsl", uo(y2), true, true, false);
      var k2 = "";
      o2.FS_createDataFile("/", "voxel_quad_ps.glsl", uo(k2), true, true, false);
      var B2 = "";
      o2.FS_createDataFile("/", "voxel_quad_vs.glsl", uo(B2), true, true, false);
      var Q2 = "";
      o2.FS_createDataFile("/", "voxel_raycast_common.glsl", uo(Q2), true, true, false);
      var C2 = "";
      o2.FS_createDataFile("/", "voxel_raycast_hdda.glsl", uo(C2), true, true, false);
      var A2 = "";
      o2.FS_createDataFile("/", "voxel_raycast_ps.glsl", uo(A2), true, true, false);
      var T2 = "ayxUgPAuy418/UKsHXWSBEtfhlpP/ghHfE2jHDG+2m3heDzsaJ256zH+br93XAopOyQT65M=";
      o2.FS_createDataFile("/", "voxel_raycast_query_ps.glsl", uo(T2), true, true, false);
      var F2 = "";
      o2.FS_createDataFile("/", "voxel_sections_common.glsl", uo(F2), true, true, false);
      var H2 = "ayxUgPAuy41g4UinBH+HF0EBgXMF+QVIZVyiWTPqw3r8cQ/BeoG5/ivlX7xbAh5rLyRCirtNpA==";
      o2.FS_createDataFile("/", "voxel_sections_depth_ps.glsl", uo(H2), true, true, false);
      var U2 = "ayxUgPAuy41g4UinBH+HF0EBgXMF+QVIZVyiWTPqw3r8cQ/BeoG5/ivlX7xbBB5rLyRCirtNpA==";
      o2.FS_createDataFile("/", "voxel_sections_depth_vs.glsl", uo(U2), true, true, false);
      var R2 = "";
      o2.FS_createDataFile("/", "voxel_sections_ps.glsl", uo(R2), true, true, false);
      var x2 = "ayxUgPAuy418/UKsHXWSBEtfhlpP/ghHfE2jHDG+2m3heDztbIeu4y3kQpB0AUMiJDtdxJRK";
      o2.FS_createDataFile("/", "voxel_sections_query_ps.glsl", uo(x2), true, true, false);
      var D2 = "ayxUgPAuy418/UKsHXWSBEtfhlpP/ghHfE2jHDG+2m3heDztbIeu4y3kQpByAUMiJDtdxJRK";
      o2.FS_createDataFile("/", "voxel_sections_query_vs.glsl", uo(D2), true, true, false);
      var W2 = "";
      o2.FS_createDataFile("/", "voxel_sections_vs.glsl", uo(W2), true, true, false);
      var b2 = "";
      o2.FS_createDataFile("/", "voxel_section_gen_common.glsl", uo(b2), true, true, false);
      var S2 = "";
      o2.FS_createDataFile("/", "voxel_section_gen_ps.glsl", uo(S2), true, true, false);
      var q2 = "";
      o2.FS_createDataFile("/", "voxel_section_gen_vs.glsl", uo(q2), true, true, false);
      var O2 = "ayxUgPAuy41g4UinBH+HF0EBgXMF+QVIZVyiWTPqw3r8cQ/Beoy/5i7VU65nGTI1O2ZWiuosjA==";
      o2.FS_createDataFile("/", "voxel_shell_back_depth_ps.glsl", uo(O2), true, true, false);
      var P2 = "ayxUgPAuy41p91mybBHaXDE74hpK5Q9OKQuwU2ut2Ur3fAbyZbu46yHhbr93XAopOyQT65M=";
      o2.FS_createDataFile("/", "voxel_shell_back_msaa_ps.glsl", uo(P2), true, true, false);
      var L2 = "";
      o2.FS_createDataFile("/", "voxel_shell_back_ps.glsl", uo(L2), true, true, false);
      var Z2 = "ayxUgPAuy418/UKsHXWSBEtfhlpP/ghHfE2jHDG+2m3heDztYYG25h3oUKxvLR02Zi9dlfVi";
      o2.FS_createDataFile("/", "voxel_shell_back_query_ps.glsl", uo(Z2), true, true, false);
      var V2 = "";
      o2.FS_createDataFile("/", "voxel_shell_dbg_ps.glsl", uo(V2), true, true, false);
      var N2 = "";
      o2.FS_createDataFile("/", "voxel_shell_vs.glsl", uo(N2), true, true, false);
      var I2 = "";
      o2.FS_createDataFile("/", "voxel_single_dda.glsl", uo(I2), true, true, false);
      var M2 = "";
      o2.FS_createDataFile("/", "voxel_tree_traversal.glsl", uo(M2), true, true, false);
    }
    o2.calledRun ? r3() : (o2.preRun || (o2.preRun = []), o2.preRun.push(r3));
  }();
  var a2, i = {};
  for (a2 in o2)
    o2.hasOwnProperty(a2) && (i[a2] = o2[a2]);
  var s, u, l, c = "./this.program", f = true, d = "";
  function E(e3) {
    return o2.locateFile ? o2.locateFile(e3, d) : d + e3;
  }
  "undefined" != typeof document && document.currentScript && (d = document.currentScript.src), t && (d = t), d = 0 !== d.indexOf("blob:") ? d.substr(0, d.replace(/[?#].*/, "").lastIndexOf("/") + 1) : "", s = function(e3) {
    var r3 = new XMLHttpRequest();
    return r3.open("GET", e3, false), r3.send(null), r3.responseText;
  }, u = function(e3, r3, t2) {
    var n3 = new XMLHttpRequest();
    n3.open("GET", e3, true), n3.responseType = "arraybuffer", n3.onload = function() {
      200 == n3.status || 0 == n3.status && n3.response ? r3(n3.response) : t2();
    }, n3.onerror = t2, n3.send(null);
  };
  var g = o2.print || console.log.bind(console), p = o2.printErr || console.warn.bind(console);
  for (a2 in i)
    i.hasOwnProperty(a2) && (o2[a2] = i[a2]);
  function h(e3, r3) {
    if ("function" == typeof WebAssembly.Function) {
      for (var t2 = { i: "i32", j: "i64", f: "f32", d: "f64" }, n3 = { parameters: [], results: "v" == r3[0] ? [] : [t2[r3[0]]] }, o3 = 1; o3 < r3.length; ++o3)
        n3.parameters.push(t2[r3[o3]]);
      return new WebAssembly.Function(n3, e3);
    }
    var a3 = [1, 0, 1, 96], i2 = r3.slice(0, 1), s2 = r3.slice(1), u2 = { i: 127, j: 126, f: 125, d: 124 };
    for (a3.push(s2.length), o3 = 0; o3 < s2.length; ++o3)
      a3.push(u2[s2[o3]]);
    "v" == i2 ? a3.push(0) : a3 = a3.concat([1, u2[i2]]), a3[1] = a3.length - 2;
    var l2 = new Uint8Array([0, 97, 115, 109, 1, 0, 0, 0].concat(a3, [2, 7, 1, 1, 101, 1, 102, 0, 0, 7, 5, 1, 1, 102, 0, 0])), c2 = new WebAssembly.Module(l2);
    return new WebAssembly.Instance(c2, { e: { f: e3 } }).exports.f;
  }
  i = null, o2.arguments && o2.arguments, o2.thisProgram && (c = o2.thisProgram), o2.quit && o2.quit;
  var m, v, w, y = [];
  function k() {
    if (y.length)
      return y.pop();
    try {
      V.grow(1);
    } catch (p2) {
      if (!(p2 instanceof RangeError))
        throw p2;
      throw "Unable to grow wasm table. Set ALLOW_TABLE_GROWTH.";
    }
    return V.length - 1;
  }
  function B(e3, r3) {
    if (!m) {
      m = /* @__PURE__ */ new WeakMap();
      for (var t2 = 0; t2 < V.length; t2++) {
        var n3 = V.get(t2);
        n3 && m.set(n3, t2);
      }
    }
    if (m.has(e3))
      return m.get(e3);
    var o3 = k();
    try {
      V.set(o3, e3);
    } catch (p2) {
      if (!(p2 instanceof TypeError))
        throw p2;
      var a3 = h(e3, r3);
      V.set(o3, a3);
    }
    return m.set(e3, o3), o3;
  }
  function Q(e3, r3) {
    return B(e3, r3);
  }
  o2.wasmBinary && (v = o2.wasmBinary), o2.noExitRuntime, "object" != typeof WebAssembly && Ee("no native wasm support detected");
  var C = false;
  function A(e3, r3) {
    e3 || Ee("Assertion failed: " + r3);
  }
  var T = "undefined" != typeof TextDecoder ? new TextDecoder("utf8") : void 0;
  function F(e3, r3, t2) {
    for (var n3 = r3 + t2, o3 = r3; e3[o3] && !(o3 >= n3); )
      ++o3;
    if (o3 - r3 > 16 && e3.subarray && T)
      return T.decode(e3.subarray(r3, o3));
    for (var a3 = ""; r3 < o3; ) {
      var i2 = e3[r3++];
      if (128 & i2) {
        var s2 = 63 & e3[r3++];
        if (192 != (224 & i2)) {
          var u2 = 63 & e3[r3++];
          if ((i2 = 224 == (240 & i2) ? (15 & i2) << 12 | s2 << 6 | u2 : (7 & i2) << 18 | s2 << 12 | u2 << 6 | 63 & e3[r3++]) < 65536)
            a3 += String.fromCharCode(i2);
          else {
            var l2 = i2 - 65536;
            a3 += String.fromCharCode(55296 | l2 >> 10, 56320 | 1023 & l2);
          }
        } else
          a3 += String.fromCharCode((31 & i2) << 6 | s2);
      } else
        a3 += String.fromCharCode(i2);
    }
    return a3;
  }
  function H(e3, r3) {
    return e3 ? F(b, e3, r3) : "";
  }
  function U(e3, r3, t2, n3) {
    if (!(n3 > 0))
      return 0;
    for (var o3 = t2, a3 = t2 + n3 - 1, i2 = 0; i2 < e3.length; ++i2) {
      var s2 = e3.charCodeAt(i2);
      if (s2 >= 55296 && s2 <= 57343 && (s2 = 65536 + ((1023 & s2) << 10) | 1023 & e3.charCodeAt(++i2)), s2 <= 127) {
        if (t2 >= a3)
          break;
        r3[t2++] = s2;
      } else if (s2 <= 2047) {
        if (t2 + 1 >= a3)
          break;
        r3[t2++] = 192 | s2 >> 6, r3[t2++] = 128 | 63 & s2;
      } else if (s2 <= 65535) {
        if (t2 + 2 >= a3)
          break;
        r3[t2++] = 224 | s2 >> 12, r3[t2++] = 128 | s2 >> 6 & 63, r3[t2++] = 128 | 63 & s2;
      } else {
        if (t2 + 3 >= a3)
          break;
        r3[t2++] = 240 | s2 >> 18, r3[t2++] = 128 | s2 >> 12 & 63, r3[t2++] = 128 | s2 >> 6 & 63, r3[t2++] = 128 | 63 & s2;
      }
    }
    return r3[t2] = 0, t2 - o3;
  }
  function R(e3, r3, t2) {
    return U(e3, b, r3, t2);
  }
  function x(e3) {
    for (var r3 = 0, t2 = 0; t2 < e3.length; ++t2) {
      var n3 = e3.charCodeAt(t2);
      n3 >= 55296 && n3 <= 57343 && (n3 = 65536 + ((1023 & n3) << 10) | 1023 & e3.charCodeAt(++t2)), n3 <= 127 ? ++r3 : r3 += n3 <= 2047 ? 2 : n3 <= 65535 ? 3 : 4;
    }
    return r3;
  }
  var D, W, b, S, q, O, P, L, Z, V, N = "undefined" != typeof TextDecoder ? new TextDecoder("utf-16le") : void 0;
  function I(e3, r3) {
    for (var t2 = e3, n3 = t2 >> 1, o3 = n3 + r3 / 2; !(n3 >= o3) && q[n3]; )
      ++n3;
    if ((t2 = n3 << 1) - e3 > 32 && N)
      return N.decode(b.subarray(e3, t2));
    for (var a3 = "", i2 = 0; !(i2 >= r3 / 2); ++i2) {
      var s2 = S[e3 + 2 * i2 >> 1];
      if (0 == s2)
        break;
      a3 += String.fromCharCode(s2);
    }
    return a3;
  }
  function M(e3, r3, t2) {
    if (void 0 === t2 && (t2 = 2147483647), t2 < 2)
      return 0;
    for (var n3 = r3, o3 = (t2 -= 2) < 2 * e3.length ? t2 / 2 : e3.length, a3 = 0; a3 < o3; ++a3) {
      var i2 = e3.charCodeAt(a3);
      S[r3 >> 1] = i2, r3 += 2;
    }
    return S[r3 >> 1] = 0, r3 - n3;
  }
  function G(e3) {
    return 2 * e3.length;
  }
  function X(e3, r3) {
    for (var t2 = 0, n3 = ""; !(t2 >= r3 / 4); ) {
      var o3 = O[e3 + 4 * t2 >> 2];
      if (0 == o3)
        break;
      if (++t2, o3 >= 65536) {
        var a3 = o3 - 65536;
        n3 += String.fromCharCode(55296 | a3 >> 10, 56320 | 1023 & a3);
      } else
        n3 += String.fromCharCode(o3);
    }
    return n3;
  }
  function J(e3, r3, t2) {
    if (void 0 === t2 && (t2 = 2147483647), t2 < 4)
      return 0;
    for (var n3 = r3, o3 = n3 + t2 - 4, a3 = 0; a3 < e3.length; ++a3) {
      var i2 = e3.charCodeAt(a3);
      if (i2 >= 55296 && i2 <= 57343 && (i2 = 65536 + ((1023 & i2) << 10) | 1023 & e3.charCodeAt(++a3)), O[r3 >> 2] = i2, (r3 += 4) + 4 > o3)
        break;
    }
    return O[r3 >> 2] = 0, r3 - n3;
  }
  function Y(e3) {
    for (var r3 = 0, t2 = 0; t2 < e3.length; ++t2) {
      var n3 = e3.charCodeAt(t2);
      n3 >= 55296 && n3 <= 57343 && ++t2, r3 += 4;
    }
    return r3;
  }
  function K(e3, r3) {
    W.set(e3, r3);
  }
  function z(e3, r3, t2) {
    for (var n3 = 0; n3 < e3.length; ++n3)
      W[r3++ >> 0] = e3.charCodeAt(n3);
    t2 || (W[r3 >> 0] = 0);
  }
  function j(e3, r3) {
    return e3 % r3 > 0 && (e3 += r3 - e3 % r3), e3;
  }
  function _(e3) {
    D = e3, o2.HEAP8 = W = new Int8Array(e3), o2.HEAP16 = S = new Int16Array(e3), o2.HEAP32 = O = new Int32Array(e3), o2.HEAPU8 = b = new Uint8Array(e3), o2.HEAPU16 = q = new Uint16Array(e3), o2.HEAPU32 = P = new Uint32Array(e3), o2.HEAPF32 = L = new Float32Array(e3), o2.HEAPF64 = Z = new Float64Array(e3);
  }
  o2.INITIAL_MEMORY;
  var $ = [], ee = [], re = [];
  function te() {
    if (o2.preRun)
      for ("function" == typeof o2.preRun && (o2.preRun = [o2.preRun]); o2.preRun.length; )
        ae(o2.preRun.shift());
    Be($);
  }
  function ne() {
    o2.noFSInit || Oe.init.initialized || Oe.init(), Oe.ignorePermissions = false, Be(ee);
  }
  function oe() {
    if (o2.postRun)
      for ("function" == typeof o2.postRun && (o2.postRun = [o2.postRun]); o2.postRun.length; )
        se(o2.postRun.shift());
    Be(re);
  }
  function ae(e3) {
    $.unshift(e3);
  }
  function ie(e3) {
    ee.unshift(e3);
  }
  function se(e3) {
    re.unshift(e3);
  }
  var ue = 0, le = null;
  function ce(e3) {
    return e3;
  }
  function fe(e3) {
    ue++, o2.monitorRunDependencies && o2.monitorRunDependencies(ue);
  }
  function de(e3) {
    if (ue--, o2.monitorRunDependencies && o2.monitorRunDependencies(ue), 0 == ue && le) {
      var r3 = le;
      le = null, r3();
    }
  }
  function Ee(e3) {
    o2.onAbort && o2.onAbort(e3), p(e3 = "Aborted(" + e3 + ")"), C = true, e3 += ". Build with -s ASSERTIONS=1 for more info.";
    var r3 = new WebAssembly.RuntimeError(e3);
    throw n2(r3), r3;
  }
  o2.preloadedImages = {}, o2.preloadedAudios = {};
  var ge, pe, he, me = "data:application/octet-stream;base64,";
  function ve(e3) {
    return e3.startsWith(me);
  }
  function we(e3) {
    try {
      if (e3 == ge && v)
        return new Uint8Array(v);
      if (!l)
        throw "both async and sync fetching of the wasm failed";
    } catch (p2) {
      Ee(p2);
    }
  }
  function ye() {
    return !v && f && "function" == typeof fetch ? fetch(ge, { credentials: "same-origin" }).then(function(e3) {
      if (!e3.ok)
        throw "failed to load wasm binary file at '" + ge + "'";
      return e3.arrayBuffer();
    }).catch(function() {
      return we(ge);
    }) : Promise.resolve().then(function() {
      return we(ge);
    });
  }
  function ke() {
    var e3 = { env: lo, wasi_snapshot_preview1: lo };
    function r3(e4, r4) {
      var t3 = e4.exports;
      o2.asm = t3, _((w = o2.asm.memory).buffer), V = o2.asm.__indirect_function_table, ie(o2.asm.__wasm_call_ctors), de();
    }
    function t2(e4) {
      r3(e4.instance);
    }
    function a3(r4) {
      return ye().then(function(r5) {
        return WebAssembly.instantiate(r5, e3);
      }).then(function(e4) {
        return e4;
      }).then(r4, function(e4) {
        p("failed to asynchronously prepare wasm: " + e4), Ee(e4);
      });
    }
    function i2() {
      return v || "function" != typeof WebAssembly.instantiateStreaming || ve(ge) || "function" != typeof fetch ? a3(t2) : fetch(ge, { credentials: "same-origin" }).then(function(r4) {
        return WebAssembly.instantiateStreaming(r4, e3).then(t2, function(e4) {
          return p("wasm streaming compile failed: " + e4), p("falling back to ArrayBuffer instantiation"), a3(t2);
        });
      });
    }
    if (fe(), o2.instantiateWasm)
      try {
        return o2.instantiateWasm(e3, r3);
      } catch (s2) {
        return p("Module.instantiateWasm callback failed with error: " + s2), false;
      }
    return i2().catch(n2), {};
  }
  function Be(e3) {
    for (; e3.length > 0; ) {
      var r3 = e3.shift();
      if ("function" != typeof r3) {
        var t2 = r3.func;
        "number" == typeof t2 ? void 0 === r3.arg ? V.get(t2)() : V.get(t2)(r3.arg) : t2(void 0 === r3.arg ? null : r3.arg);
      } else
        r3(o2);
    }
  }
  function Qe(e3, r3, t2, n3) {
    Ee("Assertion failed: " + H(e3) + ", at: " + [r3 ? H(r3) : "unknown filename", t2, n3 ? H(n3) : "unknown function"]);
  }
  function Ce(e3) {
    return fo(e3 + 16) + 16;
  }
  function Ae(e3, r3) {
  }
  function Te(e3, r3) {
    return Ae();
  }
  function Fe(e3) {
    this.excPtr = e3, this.ptr = e3 - 16, this.set_type = function(e4) {
      O[this.ptr + 4 >> 2] = e4;
    }, this.get_type = function() {
      return O[this.ptr + 4 >> 2];
    }, this.set_destructor = function(e4) {
      O[this.ptr + 8 >> 2] = e4;
    }, this.get_destructor = function() {
      return O[this.ptr + 8 >> 2];
    }, this.set_refcount = function(e4) {
      O[this.ptr >> 2] = e4;
    }, this.set_caught = function(e4) {
      e4 = e4 ? 1 : 0, W[this.ptr + 12 >> 0] = e4;
    }, this.get_caught = function() {
      return 0 != W[this.ptr + 12 >> 0];
    }, this.set_rethrown = function(e4) {
      e4 = e4 ? 1 : 0, W[this.ptr + 13 >> 0] = e4;
    }, this.get_rethrown = function() {
      return 0 != W[this.ptr + 13 >> 0];
    }, this.init = function(e4, r3) {
      this.set_type(e4), this.set_destructor(r3), this.set_refcount(0), this.set_caught(false), this.set_rethrown(false);
    }, this.add_ref = function() {
      var e4 = O[this.ptr >> 2];
      O[this.ptr >> 2] = e4 + 1;
    }, this.release_ref = function() {
      var e4 = O[this.ptr >> 2];
      return O[this.ptr >> 2] = e4 - 1, 1 === e4;
    };
  }
  function He(e3, r3, t2) {
    throw new Fe(e3).init(r3, t2), e3;
  }
  function Ue(e3) {
    return O[po() >> 2] = e3, e3;
  }
  ve(ge = "vxlLayer.wasm") || (ge = E(ge));
  var Re = { splitPath: function(e3) {
    return /^(\/?|)([\s\S]*?)((?:\.{1,2}|[^\/]+?|)(\.[^.\/]*|))(?:[\/]*)$/.exec(e3).slice(1);
  }, normalizeArray: function(e3, r3) {
    for (var t2 = 0, n3 = e3.length - 1; n3 >= 0; n3--) {
      var o3 = e3[n3];
      "." === o3 ? e3.splice(n3, 1) : ".." === o3 ? (e3.splice(n3, 1), t2++) : t2 && (e3.splice(n3, 1), t2--);
    }
    if (r3)
      for (; t2; t2--)
        e3.unshift("..");
    return e3;
  }, normalize: function(e3) {
    var r3 = "/" === e3.charAt(0), t2 = "/" === e3.substr(-1);
    return (e3 = Re.normalizeArray(e3.split("/").filter(function(e4) {
      return !!e4;
    }), !r3).join("/")) || r3 || (e3 = "."), e3 && t2 && (e3 += "/"), (r3 ? "/" : "") + e3;
  }, dirname: function(e3) {
    var r3 = Re.splitPath(e3), t2 = r3[0], n3 = r3[1];
    return t2 || n3 ? (n3 && (n3 = n3.substr(0, n3.length - 1)), t2 + n3) : ".";
  }, basename: function(e3) {
    if ("/" === e3)
      return "/";
    var r3 = (e3 = (e3 = Re.normalize(e3)).replace(/\/$/, "")).lastIndexOf("/");
    return -1 === r3 ? e3 : e3.substr(r3 + 1);
  }, extname: function(e3) {
    return Re.splitPath(e3)[3];
  }, join: function() {
    var e3 = Array.prototype.slice.call(arguments, 0);
    return Re.normalize(e3.join("/"));
  }, join2: function(e3, r3) {
    return Re.normalize(e3 + "/" + r3);
  } };
  function xe() {
    if ("object" == typeof crypto && "function" == typeof crypto.getRandomValues) {
      var e3 = new Uint8Array(1);
      return function() {
        return crypto.getRandomValues(e3), e3[0];
      };
    }
    return function() {
      Ee("randomDevice");
    };
  }
  var De = { resolve: function() {
    for (var e3 = "", r3 = false, t2 = arguments.length - 1; t2 >= -1 && !r3; t2--) {
      var n3 = t2 >= 0 ? arguments[t2] : Oe.cwd();
      if ("string" != typeof n3)
        throw new TypeError("Arguments to path.resolve must be strings");
      if (!n3)
        return "";
      e3 = n3 + "/" + e3, r3 = "/" === n3.charAt(0);
    }
    return (r3 ? "/" : "") + (e3 = Re.normalizeArray(e3.split("/").filter(function(e4) {
      return !!e4;
    }), !r3).join("/")) || ".";
  }, relative: function(e3, r3) {
    function t2(e4) {
      for (var r4 = 0; r4 < e4.length && "" === e4[r4]; r4++)
        ;
      for (var t3 = e4.length - 1; t3 >= 0 && "" === e4[t3]; t3--)
        ;
      return r4 > t3 ? [] : e4.slice(r4, t3 - r4 + 1);
    }
    e3 = De.resolve(e3).substr(1), r3 = De.resolve(r3).substr(1);
    for (var n3 = t2(e3.split("/")), o3 = t2(r3.split("/")), a3 = Math.min(n3.length, o3.length), i2 = a3, s2 = 0; s2 < a3; s2++)
      if (n3[s2] !== o3[s2]) {
        i2 = s2;
        break;
      }
    var u2 = [];
    for (s2 = i2; s2 < n3.length; s2++)
      u2.push("..");
    return (u2 = u2.concat(o3.slice(i2))).join("/");
  } }, We = { ttys: [], init: function() {
  }, shutdown: function() {
  }, register: function(e3, r3) {
    We.ttys[e3] = { input: [], output: [], ops: r3 }, Oe.registerDevice(e3, We.stream_ops);
  }, stream_ops: { open: function(e3) {
    var r3 = We.ttys[e3.node.rdev];
    if (!r3)
      throw new Oe.ErrnoError(43);
    e3.tty = r3, e3.seekable = false;
  }, close: function(e3) {
    e3.tty.ops.flush(e3.tty);
  }, flush: function(e3) {
    e3.tty.ops.flush(e3.tty);
  }, read: function(e3, r3, t2, n3, o3) {
    if (!e3.tty || !e3.tty.ops.get_char)
      throw new Oe.ErrnoError(60);
    for (var a3 = 0, i2 = 0; i2 < n3; i2++) {
      var s2;
      try {
        s2 = e3.tty.ops.get_char(e3.tty);
      } catch (u2) {
        throw new Oe.ErrnoError(29);
      }
      if (void 0 === s2 && 0 === a3)
        throw new Oe.ErrnoError(6);
      if (null == s2)
        break;
      a3++, r3[t2 + i2] = s2;
    }
    return a3 && (e3.node.timestamp = Date.now()), a3;
  }, write: function(e3, r3, t2, n3, o3) {
    if (!e3.tty || !e3.tty.ops.put_char)
      throw new Oe.ErrnoError(60);
    try {
      for (var a3 = 0; a3 < n3; a3++)
        e3.tty.ops.put_char(e3.tty, r3[t2 + a3]);
    } catch (i2) {
      throw new Oe.ErrnoError(29);
    }
    return n3 && (e3.node.timestamp = Date.now()), a3;
  } }, default_tty_ops: { get_char: function(e3) {
    if (!e3.input.length) {
      var r3 = null;
      if ("undefined" != typeof window && "function" == typeof window.prompt ? null !== (r3 = window.prompt("Input: ")) && (r3 += "\n") : "function" == typeof readline && null !== (r3 = readline()) && (r3 += "\n"), !r3)
        return null;
      e3.input = so(r3, true);
    }
    return e3.input.shift();
  }, put_char: function(e3, r3) {
    null === r3 || 10 === r3 ? (g(F(e3.output, 0)), e3.output = []) : 0 != r3 && e3.output.push(r3);
  }, flush: function(e3) {
    e3.output && e3.output.length > 0 && (g(F(e3.output, 0)), e3.output = []);
  } }, default_tty1_ops: { put_char: function(e3, r3) {
    null === r3 || 10 === r3 ? (p(F(e3.output, 0)), e3.output = []) : 0 != r3 && e3.output.push(r3);
  }, flush: function(e3) {
    e3.output && e3.output.length > 0 && (p(F(e3.output, 0)), e3.output = []);
  } } };
  function be(e3) {
    Ee();
  }
  var Se = { ops_table: null, mount: function(e3) {
    return Se.createNode(null, "/", 16895, 0);
  }, createNode: function(e3, r3, t2, n3) {
    if (Oe.isBlkdev(t2) || Oe.isFIFO(t2))
      throw new Oe.ErrnoError(63);
    Se.ops_table || (Se.ops_table = { dir: { node: { getattr: Se.node_ops.getattr, setattr: Se.node_ops.setattr, lookup: Se.node_ops.lookup, mknod: Se.node_ops.mknod, rename: Se.node_ops.rename, unlink: Se.node_ops.unlink, rmdir: Se.node_ops.rmdir, readdir: Se.node_ops.readdir, symlink: Se.node_ops.symlink }, stream: { llseek: Se.stream_ops.llseek } }, file: { node: { getattr: Se.node_ops.getattr, setattr: Se.node_ops.setattr }, stream: { llseek: Se.stream_ops.llseek, read: Se.stream_ops.read, write: Se.stream_ops.write, allocate: Se.stream_ops.allocate, mmap: Se.stream_ops.mmap, msync: Se.stream_ops.msync } }, link: { node: { getattr: Se.node_ops.getattr, setattr: Se.node_ops.setattr, readlink: Se.node_ops.readlink }, stream: {} }, chrdev: { node: { getattr: Se.node_ops.getattr, setattr: Se.node_ops.setattr }, stream: Oe.chrdev_stream_ops } });
    var o3 = Oe.createNode(e3, r3, t2, n3);
    return Oe.isDir(o3.mode) ? (o3.node_ops = Se.ops_table.dir.node, o3.stream_ops = Se.ops_table.dir.stream, o3.contents = {}) : Oe.isFile(o3.mode) ? (o3.node_ops = Se.ops_table.file.node, o3.stream_ops = Se.ops_table.file.stream, o3.usedBytes = 0, o3.contents = null) : Oe.isLink(o3.mode) ? (o3.node_ops = Se.ops_table.link.node, o3.stream_ops = Se.ops_table.link.stream) : Oe.isChrdev(o3.mode) && (o3.node_ops = Se.ops_table.chrdev.node, o3.stream_ops = Se.ops_table.chrdev.stream), o3.timestamp = Date.now(), e3 && (e3.contents[r3] = o3, e3.timestamp = o3.timestamp), o3;
  }, getFileDataAsTypedArray: function(e3) {
    return e3.contents ? e3.contents.subarray ? e3.contents.subarray(0, e3.usedBytes) : new Uint8Array(e3.contents) : new Uint8Array(0);
  }, expandFileStorage: function(e3, r3) {
    var t2 = e3.contents ? e3.contents.length : 0;
    if (!(t2 >= r3)) {
      var n3 = 1048576;
      r3 = Math.max(r3, t2 * (t2 < n3 ? 2 : 1.125) >>> 0), 0 != t2 && (r3 = Math.max(r3, 256));
      var o3 = e3.contents;
      e3.contents = new Uint8Array(r3), e3.usedBytes > 0 && e3.contents.set(o3.subarray(0, e3.usedBytes), 0);
    }
  }, resizeFileStorage: function(e3, r3) {
    if (e3.usedBytes != r3)
      if (0 == r3)
        e3.contents = null, e3.usedBytes = 0;
      else {
        var t2 = e3.contents;
        e3.contents = new Uint8Array(r3), t2 && e3.contents.set(t2.subarray(0, Math.min(r3, e3.usedBytes))), e3.usedBytes = r3;
      }
  }, node_ops: { getattr: function(e3) {
    var r3 = {};
    return r3.dev = Oe.isChrdev(e3.mode) ? e3.id : 1, r3.ino = e3.id, r3.mode = e3.mode, r3.nlink = 1, r3.uid = 0, r3.gid = 0, r3.rdev = e3.rdev, Oe.isDir(e3.mode) ? r3.size = 4096 : Oe.isFile(e3.mode) ? r3.size = e3.usedBytes : Oe.isLink(e3.mode) ? r3.size = e3.link.length : r3.size = 0, r3.atime = new Date(e3.timestamp), r3.mtime = new Date(e3.timestamp), r3.ctime = new Date(e3.timestamp), r3.blksize = 4096, r3.blocks = Math.ceil(r3.size / r3.blksize), r3;
  }, setattr: function(e3, r3) {
    void 0 !== r3.mode && (e3.mode = r3.mode), void 0 !== r3.timestamp && (e3.timestamp = r3.timestamp), void 0 !== r3.size && Se.resizeFileStorage(e3, r3.size);
  }, lookup: function(e3, r3) {
    throw Oe.genericErrors[44];
  }, mknod: function(e3, r3, t2, n3) {
    return Se.createNode(e3, r3, t2, n3);
  }, rename: function(e3, r3, t2) {
    if (Oe.isDir(e3.mode)) {
      var n3;
      try {
        n3 = Oe.lookupNode(r3, t2);
      } catch (a3) {
      }
      if (n3)
        for (var o3 in n3.contents)
          throw new Oe.ErrnoError(55);
    }
    delete e3.parent.contents[e3.name], e3.parent.timestamp = Date.now(), e3.name = t2, r3.contents[t2] = e3, r3.timestamp = e3.parent.timestamp, e3.parent = r3;
  }, unlink: function(e3, r3) {
    delete e3.contents[r3], e3.timestamp = Date.now();
  }, rmdir: function(e3, r3) {
    var t2 = Oe.lookupNode(e3, r3);
    for (var n3 in t2.contents)
      throw new Oe.ErrnoError(55);
    delete e3.contents[r3], e3.timestamp = Date.now();
  }, readdir: function(e3) {
    var r3 = [".", ".."];
    for (var t2 in e3.contents)
      e3.contents.hasOwnProperty(t2) && r3.push(t2);
    return r3;
  }, symlink: function(e3, r3, t2) {
    var n3 = Se.createNode(e3, r3, 41471, 0);
    return n3.link = t2, n3;
  }, readlink: function(e3) {
    if (!Oe.isLink(e3.mode))
      throw new Oe.ErrnoError(28);
    return e3.link;
  } }, stream_ops: { read: function(e3, r3, t2, n3, o3) {
    var a3 = e3.node.contents;
    if (o3 >= e3.node.usedBytes)
      return 0;
    var i2 = Math.min(e3.node.usedBytes - o3, n3);
    if (i2 > 8 && a3.subarray)
      r3.set(a3.subarray(o3, o3 + i2), t2);
    else
      for (var s2 = 0; s2 < i2; s2++)
        r3[t2 + s2] = a3[o3 + s2];
    return i2;
  }, write: function(e3, r3, t2, n3, o3, a3) {
    if (r3.buffer === W.buffer && (a3 = false), !n3)
      return 0;
    var i2 = e3.node;
    if (i2.timestamp = Date.now(), r3.subarray && (!i2.contents || i2.contents.subarray)) {
      if (a3)
        return i2.contents = r3.subarray(t2, t2 + n3), i2.usedBytes = n3, n3;
      if (0 === i2.usedBytes && 0 === o3)
        return i2.contents = r3.slice(t2, t2 + n3), i2.usedBytes = n3, n3;
      if (o3 + n3 <= i2.usedBytes)
        return i2.contents.set(r3.subarray(t2, t2 + n3), o3), n3;
    }
    if (Se.expandFileStorage(i2, o3 + n3), i2.contents.subarray && r3.subarray)
      i2.contents.set(r3.subarray(t2, t2 + n3), o3);
    else
      for (var s2 = 0; s2 < n3; s2++)
        i2.contents[o3 + s2] = r3[t2 + s2];
    return i2.usedBytes = Math.max(i2.usedBytes, o3 + n3), n3;
  }, llseek: function(e3, r3, t2) {
    var n3 = r3;
    if (1 === t2 ? n3 += e3.position : 2 === t2 && Oe.isFile(e3.node.mode) && (n3 += e3.node.usedBytes), n3 < 0)
      throw new Oe.ErrnoError(28);
    return n3;
  }, allocate: function(e3, r3, t2) {
    Se.expandFileStorage(e3.node, r3 + t2), e3.node.usedBytes = Math.max(e3.node.usedBytes, r3 + t2);
  }, mmap: function(e3, r3, t2, n3, o3, a3) {
    if (0 !== r3)
      throw new Oe.ErrnoError(28);
    if (!Oe.isFile(e3.node.mode))
      throw new Oe.ErrnoError(43);
    var i2, s2, u2 = e3.node.contents;
    if (2 & a3 || u2.buffer !== D) {
      if ((n3 > 0 || n3 + t2 < u2.length) && (u2 = u2.subarray ? u2.subarray(n3, n3 + t2) : Array.prototype.slice.call(u2, n3, n3 + t2)), s2 = true, !(i2 = be()))
        throw new Oe.ErrnoError(48);
      W.set(u2, i2);
    } else
      s2 = false, i2 = u2.byteOffset;
    return { ptr: i2, allocated: s2 };
  }, msync: function(e3, r3, t2, n3, o3) {
    if (!Oe.isFile(e3.node.mode))
      throw new Oe.ErrnoError(43);
    return 2 & o3 || Se.stream_ops.write(e3, r3, 0, n3, t2, false), 0;
  } } };
  function qe(e3, r3, t2, n3) {
    var o3 = n3 ? "" : ce("al " + e3);
    u(e3, function(t3) {
      A(t3, 'Loading data file "' + e3 + '" failed (no arrayBuffer).'), r3(new Uint8Array(t3)), o3 && de();
    }, function(r4) {
      if (!t2)
        throw 'Loading data file "' + e3 + '" failed.';
      t2();
    }), o3 && fe();
  }
  var Oe = { root: null, mounts: [], devices: {}, streams: [], nextInode: 1, nameTable: null, currentPath: "/", initialized: false, ignorePermissions: true, ErrnoError: null, genericErrors: {}, filesystems: null, syncFSRequests: 0, lookupPath: function(e3, r3) {
    if (r3 = r3 || {}, !(e3 = De.resolve(Oe.cwd(), e3)))
      return { path: "", node: null };
    var t2 = { follow_mount: true, recurse_count: 0 };
    for (var n3 in t2)
      void 0 === r3[n3] && (r3[n3] = t2[n3]);
    if (r3.recurse_count > 8)
      throw new Oe.ErrnoError(32);
    for (var o3 = Re.normalizeArray(e3.split("/").filter(function(e4) {
      return !!e4;
    }), false), a3 = Oe.root, i2 = "/", s2 = 0; s2 < o3.length; s2++) {
      var u2 = s2 === o3.length - 1;
      if (u2 && r3.parent)
        break;
      if (a3 = Oe.lookupNode(a3, o3[s2]), i2 = Re.join2(i2, o3[s2]), Oe.isMountpoint(a3) && (!u2 || u2 && r3.follow_mount) && (a3 = a3.mounted.root), !u2 || r3.follow)
        for (var l2 = 0; Oe.isLink(a3.mode); ) {
          var c2 = Oe.readlink(i2);
          if (i2 = De.resolve(Re.dirname(i2), c2), a3 = Oe.lookupPath(i2, { recurse_count: r3.recurse_count }).node, l2++ > 40)
            throw new Oe.ErrnoError(32);
        }
    }
    return { path: i2, node: a3 };
  }, getPath: function(e3) {
    for (var r3; ; ) {
      if (Oe.isRoot(e3)) {
        var t2 = e3.mount.mountpoint;
        return r3 ? "/" !== t2[t2.length - 1] ? t2 + "/" + r3 : t2 + r3 : t2;
      }
      r3 = r3 ? e3.name + "/" + r3 : e3.name, e3 = e3.parent;
    }
  }, hashName: function(e3, r3) {
    for (var t2 = 0, n3 = 0; n3 < r3.length; n3++)
      t2 = (t2 << 5) - t2 + r3.charCodeAt(n3) | 0;
    return (e3 + t2 >>> 0) % Oe.nameTable.length;
  }, hashAddNode: function(e3) {
    var r3 = Oe.hashName(e3.parent.id, e3.name);
    e3.name_next = Oe.nameTable[r3], Oe.nameTable[r3] = e3;
  }, hashRemoveNode: function(e3) {
    var r3 = Oe.hashName(e3.parent.id, e3.name);
    if (Oe.nameTable[r3] === e3)
      Oe.nameTable[r3] = e3.name_next;
    else
      for (var t2 = Oe.nameTable[r3]; t2; ) {
        if (t2.name_next === e3) {
          t2.name_next = e3.name_next;
          break;
        }
        t2 = t2.name_next;
      }
  }, lookupNode: function(e3, r3) {
    var t2 = Oe.mayLookup(e3);
    if (t2)
      throw new Oe.ErrnoError(t2, e3);
    for (var n3 = Oe.hashName(e3.id, r3), o3 = Oe.nameTable[n3]; o3; o3 = o3.name_next) {
      var a3 = o3.name;
      if (o3.parent.id === e3.id && a3 === r3)
        return o3;
    }
    return Oe.lookup(e3, r3);
  }, createNode: function(e3, r3, t2, n3) {
    var o3 = new Oe.FSNode(e3, r3, t2, n3);
    return Oe.hashAddNode(o3), o3;
  }, destroyNode: function(e3) {
    Oe.hashRemoveNode(e3);
  }, isRoot: function(e3) {
    return e3 === e3.parent;
  }, isMountpoint: function(e3) {
    return !!e3.mounted;
  }, isFile: function(e3) {
    return 32768 == (61440 & e3);
  }, isDir: function(e3) {
    return 16384 == (61440 & e3);
  }, isLink: function(e3) {
    return 40960 == (61440 & e3);
  }, isChrdev: function(e3) {
    return 8192 == (61440 & e3);
  }, isBlkdev: function(e3) {
    return 24576 == (61440 & e3);
  }, isFIFO: function(e3) {
    return 4096 == (61440 & e3);
  }, isSocket: function(e3) {
    return 49152 == (49152 & e3);
  }, flagModes: { r: 0, "r+": 2, w: 577, "w+": 578, a: 1089, "a+": 1090 }, modeStringToFlags: function(e3) {
    var r3 = Oe.flagModes[e3];
    if (void 0 === r3)
      throw new Error("Unknown file open mode: " + e3);
    return r3;
  }, flagsToPermissionString: function(e3) {
    var r3 = ["r", "w", "rw"][3 & e3];
    return 512 & e3 && (r3 += "w"), r3;
  }, nodePermissions: function(e3, r3) {
    return Oe.ignorePermissions || (!r3.includes("r") || 292 & e3.mode) && (!r3.includes("w") || 146 & e3.mode) && (!r3.includes("x") || 73 & e3.mode) ? 0 : 2;
  }, mayLookup: function(e3) {
    var r3 = Oe.nodePermissions(e3, "x");
    return r3 || (e3.node_ops.lookup ? 0 : 2);
  }, mayCreate: function(e3, r3) {
    try {
      return Oe.lookupNode(e3, r3), 20;
    } catch (t2) {
    }
    return Oe.nodePermissions(e3, "wx");
  }, mayDelete: function(e3, r3, t2) {
    var n3;
    try {
      n3 = Oe.lookupNode(e3, r3);
    } catch (a3) {
      return a3.errno;
    }
    var o3 = Oe.nodePermissions(e3, "wx");
    if (o3)
      return o3;
    if (t2) {
      if (!Oe.isDir(n3.mode))
        return 54;
      if (Oe.isRoot(n3) || Oe.getPath(n3) === Oe.cwd())
        return 10;
    } else if (Oe.isDir(n3.mode))
      return 31;
    return 0;
  }, mayOpen: function(e3, r3) {
    return e3 ? Oe.isLink(e3.mode) ? 32 : Oe.isDir(e3.mode) && ("r" !== Oe.flagsToPermissionString(r3) || 512 & r3) ? 31 : Oe.nodePermissions(e3, Oe.flagsToPermissionString(r3)) : 44;
  }, MAX_OPEN_FDS: 4096, nextfd: function(e3, r3) {
    e3 = e3 || 0, r3 = r3 || Oe.MAX_OPEN_FDS;
    for (var t2 = e3; t2 <= r3; t2++)
      if (!Oe.streams[t2])
        return t2;
    throw new Oe.ErrnoError(33);
  }, getStream: function(e3) {
    return Oe.streams[e3];
  }, createStream: function(e3, r3, t2) {
    Oe.FSStream || (Oe.FSStream = function() {
    }, Oe.FSStream.prototype = { object: { get: function() {
      return this.node;
    }, set: function(e4) {
      this.node = e4;
    } }, isRead: { get: function() {
      return 1 != (2097155 & this.flags);
    } }, isWrite: { get: function() {
      return 0 != (2097155 & this.flags);
    } }, isAppend: { get: function() {
      return 1024 & this.flags;
    } } });
    var n3 = new Oe.FSStream();
    for (var o3 in e3)
      n3[o3] = e3[o3];
    e3 = n3;
    var a3 = Oe.nextfd(r3, t2);
    return e3.fd = a3, Oe.streams[a3] = e3, e3;
  }, closeStream: function(e3) {
    Oe.streams[e3] = null;
  }, chrdev_stream_ops: { open: function(e3) {
    var r3 = Oe.getDevice(e3.node.rdev);
    e3.stream_ops = r3.stream_ops, e3.stream_ops.open && e3.stream_ops.open(e3);
  }, llseek: function() {
    throw new Oe.ErrnoError(70);
  } }, major: function(e3) {
    return e3 >> 8;
  }, minor: function(e3) {
    return 255 & e3;
  }, makedev: function(e3, r3) {
    return e3 << 8 | r3;
  }, registerDevice: function(e3, r3) {
    Oe.devices[e3] = { stream_ops: r3 };
  }, getDevice: function(e3) {
    return Oe.devices[e3];
  }, getMounts: function(e3) {
    for (var r3 = [], t2 = [e3]; t2.length; ) {
      var n3 = t2.pop();
      r3.push(n3), t2.push.apply(t2, n3.mounts);
    }
    return r3;
  }, syncfs: function(e3, r3) {
    "function" == typeof e3 && (r3 = e3, e3 = false), Oe.syncFSRequests++, Oe.syncFSRequests > 1 && p("warning: " + Oe.syncFSRequests + " FS.syncfs operations in flight at once, probably just doing extra work");
    var t2 = Oe.getMounts(Oe.root.mount), n3 = 0;
    function o3(e4) {
      return Oe.syncFSRequests--, r3(e4);
    }
    function a3(e4) {
      if (e4)
        return a3.errored ? void 0 : (a3.errored = true, o3(e4));
      ++n3 >= t2.length && o3(null);
    }
    t2.forEach(function(r4) {
      if (!r4.type.syncfs)
        return a3(null);
      r4.type.syncfs(r4, e3, a3);
    });
  }, mount: function(e3, r3, t2) {
    var n3, o3 = "/" === t2, a3 = !t2;
    if (o3 && Oe.root)
      throw new Oe.ErrnoError(10);
    if (!o3 && !a3) {
      var i2 = Oe.lookupPath(t2, { follow_mount: false });
      if (t2 = i2.path, n3 = i2.node, Oe.isMountpoint(n3))
        throw new Oe.ErrnoError(10);
      if (!Oe.isDir(n3.mode))
        throw new Oe.ErrnoError(54);
    }
    var s2 = { type: e3, opts: r3, mountpoint: t2, mounts: [] }, u2 = e3.mount(s2);
    return u2.mount = s2, s2.root = u2, o3 ? Oe.root = u2 : n3 && (n3.mounted = s2, n3.mount && n3.mount.mounts.push(s2)), u2;
  }, unmount: function(e3) {
    var r3 = Oe.lookupPath(e3, { follow_mount: false });
    if (!Oe.isMountpoint(r3.node))
      throw new Oe.ErrnoError(28);
    var t2 = r3.node, n3 = t2.mounted, o3 = Oe.getMounts(n3);
    Object.keys(Oe.nameTable).forEach(function(e4) {
      for (var r4 = Oe.nameTable[e4]; r4; ) {
        var t3 = r4.name_next;
        o3.includes(r4.mount) && Oe.destroyNode(r4), r4 = t3;
      }
    }), t2.mounted = null;
    var a3 = t2.mount.mounts.indexOf(n3);
    t2.mount.mounts.splice(a3, 1);
  }, lookup: function(e3, r3) {
    return e3.node_ops.lookup(e3, r3);
  }, mknod: function(e3, r3, t2) {
    var n3 = Oe.lookupPath(e3, { parent: true }).node, o3 = Re.basename(e3);
    if (!o3 || "." === o3 || ".." === o3)
      throw new Oe.ErrnoError(28);
    var a3 = Oe.mayCreate(n3, o3);
    if (a3)
      throw new Oe.ErrnoError(a3);
    if (!n3.node_ops.mknod)
      throw new Oe.ErrnoError(63);
    return n3.node_ops.mknod(n3, o3, r3, t2);
  }, create: function(e3, r3) {
    return r3 = void 0 !== r3 ? r3 : 438, r3 &= 4095, r3 |= 32768, Oe.mknod(e3, r3, 0);
  }, mkdir: function(e3, r3) {
    return r3 = void 0 !== r3 ? r3 : 511, r3 &= 1023, r3 |= 16384, Oe.mknod(e3, r3, 0);
  }, mkdirTree: function(e3, r3) {
    for (var t2 = e3.split("/"), n3 = "", o3 = 0; o3 < t2.length; ++o3)
      if (t2[o3]) {
        n3 += "/" + t2[o3];
        try {
          Oe.mkdir(n3, r3);
        } catch (a3) {
          if (20 != a3.errno)
            throw a3;
        }
      }
  }, mkdev: function(e3, r3, t2) {
    return void 0 === t2 && (t2 = r3, r3 = 438), r3 |= 8192, Oe.mknod(e3, r3, t2);
  }, symlink: function(e3, r3) {
    if (!De.resolve(e3))
      throw new Oe.ErrnoError(44);
    var t2 = Oe.lookupPath(r3, { parent: true }).node;
    if (!t2)
      throw new Oe.ErrnoError(44);
    var n3 = Re.basename(r3), o3 = Oe.mayCreate(t2, n3);
    if (o3)
      throw new Oe.ErrnoError(o3);
    if (!t2.node_ops.symlink)
      throw new Oe.ErrnoError(63);
    return t2.node_ops.symlink(t2, n3, e3);
  }, rename: function(e3, r3) {
    var t2, n3, o3 = Re.dirname(e3), a3 = Re.dirname(r3), i2 = Re.basename(e3), s2 = Re.basename(r3);
    if (t2 = Oe.lookupPath(e3, { parent: true }).node, n3 = Oe.lookupPath(r3, { parent: true }).node, !t2 || !n3)
      throw new Oe.ErrnoError(44);
    if (t2.mount !== n3.mount)
      throw new Oe.ErrnoError(75);
    var u2, l2 = Oe.lookupNode(t2, i2), c2 = De.relative(e3, a3);
    if ("." !== c2.charAt(0))
      throw new Oe.ErrnoError(28);
    if ("." !== (c2 = De.relative(r3, o3)).charAt(0))
      throw new Oe.ErrnoError(55);
    try {
      u2 = Oe.lookupNode(n3, s2);
    } catch (E2) {
    }
    if (l2 !== u2) {
      var f2 = Oe.isDir(l2.mode), d2 = Oe.mayDelete(t2, i2, f2);
      if (d2)
        throw new Oe.ErrnoError(d2);
      if (d2 = u2 ? Oe.mayDelete(n3, s2, f2) : Oe.mayCreate(n3, s2))
        throw new Oe.ErrnoError(d2);
      if (!t2.node_ops.rename)
        throw new Oe.ErrnoError(63);
      if (Oe.isMountpoint(l2) || u2 && Oe.isMountpoint(u2))
        throw new Oe.ErrnoError(10);
      if (n3 !== t2 && (d2 = Oe.nodePermissions(t2, "w")))
        throw new Oe.ErrnoError(d2);
      Oe.hashRemoveNode(l2);
      try {
        t2.node_ops.rename(l2, n3, s2);
      } catch (E2) {
        throw E2;
      } finally {
        Oe.hashAddNode(l2);
      }
    }
  }, rmdir: function(e3) {
    var r3 = Oe.lookupPath(e3, { parent: true }).node, t2 = Re.basename(e3), n3 = Oe.lookupNode(r3, t2), o3 = Oe.mayDelete(r3, t2, true);
    if (o3)
      throw new Oe.ErrnoError(o3);
    if (!r3.node_ops.rmdir)
      throw new Oe.ErrnoError(63);
    if (Oe.isMountpoint(n3))
      throw new Oe.ErrnoError(10);
    r3.node_ops.rmdir(r3, t2), Oe.destroyNode(n3);
  }, readdir: function(e3) {
    var r3 = Oe.lookupPath(e3, { follow: true }).node;
    if (!r3.node_ops.readdir)
      throw new Oe.ErrnoError(54);
    return r3.node_ops.readdir(r3);
  }, unlink: function(e3) {
    var r3 = Oe.lookupPath(e3, { parent: true }).node, t2 = Re.basename(e3), n3 = Oe.lookupNode(r3, t2), o3 = Oe.mayDelete(r3, t2, false);
    if (o3)
      throw new Oe.ErrnoError(o3);
    if (!r3.node_ops.unlink)
      throw new Oe.ErrnoError(63);
    if (Oe.isMountpoint(n3))
      throw new Oe.ErrnoError(10);
    r3.node_ops.unlink(r3, t2), Oe.destroyNode(n3);
  }, readlink: function(e3) {
    var r3 = Oe.lookupPath(e3).node;
    if (!r3)
      throw new Oe.ErrnoError(44);
    if (!r3.node_ops.readlink)
      throw new Oe.ErrnoError(28);
    return De.resolve(Oe.getPath(r3.parent), r3.node_ops.readlink(r3));
  }, stat: function(e3, r3) {
    var t2 = Oe.lookupPath(e3, { follow: !r3 }).node;
    if (!t2)
      throw new Oe.ErrnoError(44);
    if (!t2.node_ops.getattr)
      throw new Oe.ErrnoError(63);
    return t2.node_ops.getattr(t2);
  }, lstat: function(e3) {
    return Oe.stat(e3, true);
  }, chmod: function(e3, r3, t2) {
    var n3;
    if (!(n3 = "string" == typeof e3 ? Oe.lookupPath(e3, { follow: !t2 }).node : e3).node_ops.setattr)
      throw new Oe.ErrnoError(63);
    n3.node_ops.setattr(n3, { mode: 4095 & r3 | -4096 & n3.mode, timestamp: Date.now() });
  }, lchmod: function(e3, r3) {
    Oe.chmod(e3, r3, true);
  }, fchmod: function(e3, r3) {
    var t2 = Oe.getStream(e3);
    if (!t2)
      throw new Oe.ErrnoError(8);
    Oe.chmod(t2.node, r3);
  }, chown: function(e3, r3, t2, n3) {
    var o3;
    if (!(o3 = "string" == typeof e3 ? Oe.lookupPath(e3, { follow: !n3 }).node : e3).node_ops.setattr)
      throw new Oe.ErrnoError(63);
    o3.node_ops.setattr(o3, { timestamp: Date.now() });
  }, lchown: function(e3, r3, t2) {
    Oe.chown(e3, r3, t2, true);
  }, fchown: function(e3, r3, t2) {
    var n3 = Oe.getStream(e3);
    if (!n3)
      throw new Oe.ErrnoError(8);
    Oe.chown(n3.node, r3, t2);
  }, truncate: function(e3, r3) {
    if (r3 < 0)
      throw new Oe.ErrnoError(28);
    var t2;
    if (!(t2 = "string" == typeof e3 ? Oe.lookupPath(e3, { follow: true }).node : e3).node_ops.setattr)
      throw new Oe.ErrnoError(63);
    if (Oe.isDir(t2.mode))
      throw new Oe.ErrnoError(31);
    if (!Oe.isFile(t2.mode))
      throw new Oe.ErrnoError(28);
    var n3 = Oe.nodePermissions(t2, "w");
    if (n3)
      throw new Oe.ErrnoError(n3);
    t2.node_ops.setattr(t2, { size: r3, timestamp: Date.now() });
  }, ftruncate: function(e3, r3) {
    var t2 = Oe.getStream(e3);
    if (!t2)
      throw new Oe.ErrnoError(8);
    if (0 == (2097155 & t2.flags))
      throw new Oe.ErrnoError(28);
    Oe.truncate(t2.node, r3);
  }, utime: function(e3, r3, t2) {
    var n3 = Oe.lookupPath(e3, { follow: true }).node;
    n3.node_ops.setattr(n3, { timestamp: Math.max(r3, t2) });
  }, open: function(e3, r3, t2, n3, a3) {
    if ("" === e3)
      throw new Oe.ErrnoError(44);
    var i2;
    if (t2 = void 0 === t2 ? 438 : t2, t2 = 64 & (r3 = "string" == typeof r3 ? Oe.modeStringToFlags(r3) : r3) ? 4095 & t2 | 32768 : 0, "object" == typeof e3)
      i2 = e3;
    else {
      e3 = Re.normalize(e3);
      try {
        i2 = Oe.lookupPath(e3, { follow: !(131072 & r3) }).node;
      } catch (c2) {
      }
    }
    var s2 = false;
    if (64 & r3)
      if (i2) {
        if (128 & r3)
          throw new Oe.ErrnoError(20);
      } else
        i2 = Oe.mknod(e3, t2, 0), s2 = true;
    if (!i2)
      throw new Oe.ErrnoError(44);
    if (Oe.isChrdev(i2.mode) && (r3 &= -513), 65536 & r3 && !Oe.isDir(i2.mode))
      throw new Oe.ErrnoError(54);
    if (!s2) {
      var u2 = Oe.mayOpen(i2, r3);
      if (u2)
        throw new Oe.ErrnoError(u2);
    }
    512 & r3 && Oe.truncate(i2, 0), r3 &= -131713;
    var l2 = Oe.createStream({ node: i2, path: Oe.getPath(i2), flags: r3, seekable: true, position: 0, stream_ops: i2.stream_ops, ungotten: [], error: false }, n3, a3);
    return l2.stream_ops.open && l2.stream_ops.open(l2), !o2.logReadFiles || 1 & r3 || (Oe.readFiles || (Oe.readFiles = {}), e3 in Oe.readFiles || (Oe.readFiles[e3] = 1)), l2;
  }, close: function(e3) {
    if (Oe.isClosed(e3))
      throw new Oe.ErrnoError(8);
    e3.getdents && (e3.getdents = null);
    try {
      e3.stream_ops.close && e3.stream_ops.close(e3);
    } catch (r3) {
      throw r3;
    } finally {
      Oe.closeStream(e3.fd);
    }
    e3.fd = null;
  }, isClosed: function(e3) {
    return null === e3.fd;
  }, llseek: function(e3, r3, t2) {
    if (Oe.isClosed(e3))
      throw new Oe.ErrnoError(8);
    if (!e3.seekable || !e3.stream_ops.llseek)
      throw new Oe.ErrnoError(70);
    if (0 != t2 && 1 != t2 && 2 != t2)
      throw new Oe.ErrnoError(28);
    return e3.position = e3.stream_ops.llseek(e3, r3, t2), e3.ungotten = [], e3.position;
  }, read: function(e3, r3, t2, n3, o3) {
    if (n3 < 0 || o3 < 0)
      throw new Oe.ErrnoError(28);
    if (Oe.isClosed(e3))
      throw new Oe.ErrnoError(8);
    if (1 == (2097155 & e3.flags))
      throw new Oe.ErrnoError(8);
    if (Oe.isDir(e3.node.mode))
      throw new Oe.ErrnoError(31);
    if (!e3.stream_ops.read)
      throw new Oe.ErrnoError(28);
    var a3 = void 0 !== o3;
    if (a3) {
      if (!e3.seekable)
        throw new Oe.ErrnoError(70);
    } else
      o3 = e3.position;
    var i2 = e3.stream_ops.read(e3, r3, t2, n3, o3);
    return a3 || (e3.position += i2), i2;
  }, write: function(e3, r3, t2, n3, o3, a3) {
    if (n3 < 0 || o3 < 0)
      throw new Oe.ErrnoError(28);
    if (Oe.isClosed(e3))
      throw new Oe.ErrnoError(8);
    if (0 == (2097155 & e3.flags))
      throw new Oe.ErrnoError(8);
    if (Oe.isDir(e3.node.mode))
      throw new Oe.ErrnoError(31);
    if (!e3.stream_ops.write)
      throw new Oe.ErrnoError(28);
    e3.seekable && 1024 & e3.flags && Oe.llseek(e3, 0, 2);
    var i2 = void 0 !== o3;
    if (i2) {
      if (!e3.seekable)
        throw new Oe.ErrnoError(70);
    } else
      o3 = e3.position;
    var s2 = e3.stream_ops.write(e3, r3, t2, n3, o3, a3);
    return i2 || (e3.position += s2), s2;
  }, allocate: function(e3, r3, t2) {
    if (Oe.isClosed(e3))
      throw new Oe.ErrnoError(8);
    if (r3 < 0 || t2 <= 0)
      throw new Oe.ErrnoError(28);
    if (0 == (2097155 & e3.flags))
      throw new Oe.ErrnoError(8);
    if (!Oe.isFile(e3.node.mode) && !Oe.isDir(e3.node.mode))
      throw new Oe.ErrnoError(43);
    if (!e3.stream_ops.allocate)
      throw new Oe.ErrnoError(138);
    e3.stream_ops.allocate(e3, r3, t2);
  }, mmap: function(e3, r3, t2, n3, o3, a3) {
    if (0 != (2 & o3) && 0 == (2 & a3) && 2 != (2097155 & e3.flags))
      throw new Oe.ErrnoError(2);
    if (1 == (2097155 & e3.flags))
      throw new Oe.ErrnoError(2);
    if (!e3.stream_ops.mmap)
      throw new Oe.ErrnoError(43);
    return e3.stream_ops.mmap(e3, r3, t2, n3, o3, a3);
  }, msync: function(e3, r3, t2, n3, o3) {
    return e3 && e3.stream_ops.msync ? e3.stream_ops.msync(e3, r3, t2, n3, o3) : 0;
  }, munmap: function(e3) {
    return 0;
  }, ioctl: function(e3, r3, t2) {
    if (!e3.stream_ops.ioctl)
      throw new Oe.ErrnoError(59);
    return e3.stream_ops.ioctl(e3, r3, t2);
  }, readFile: function(e3, r3) {
    if ((r3 = r3 || {}).flags = r3.flags || 0, r3.encoding = r3.encoding || "binary", "utf8" !== r3.encoding && "binary" !== r3.encoding)
      throw new Error('Invalid encoding type "' + r3.encoding + '"');
    var t2, n3 = Oe.open(e3, r3.flags), o3 = Oe.stat(e3).size, a3 = new Uint8Array(o3);
    return Oe.read(n3, a3, 0, o3, 0), "utf8" === r3.encoding ? t2 = F(a3, 0) : "binary" === r3.encoding && (t2 = a3), Oe.close(n3), t2;
  }, writeFile: function(e3, r3, t2) {
    (t2 = t2 || {}).flags = t2.flags || 577;
    var n3 = Oe.open(e3, t2.flags, t2.mode);
    if ("string" == typeof r3) {
      var o3 = new Uint8Array(x(r3) + 1), a3 = U(r3, o3, 0, o3.length);
      Oe.write(n3, o3, 0, a3, void 0, t2.canOwn);
    } else {
      if (!ArrayBuffer.isView(r3))
        throw new Error("Unsupported data type");
      Oe.write(n3, r3, 0, r3.byteLength, void 0, t2.canOwn);
    }
    Oe.close(n3);
  }, cwd: function() {
    return Oe.currentPath;
  }, chdir: function(e3) {
    var r3 = Oe.lookupPath(e3, { follow: true });
    if (null === r3.node)
      throw new Oe.ErrnoError(44);
    if (!Oe.isDir(r3.node.mode))
      throw new Oe.ErrnoError(54);
    var t2 = Oe.nodePermissions(r3.node, "x");
    if (t2)
      throw new Oe.ErrnoError(t2);
    Oe.currentPath = r3.path;
  }, createDefaultDirectories: function() {
    Oe.mkdir("/tmp"), Oe.mkdir("/home"), Oe.mkdir("/home/web_user");
  }, createDefaultDevices: function() {
    Oe.mkdir("/dev"), Oe.registerDevice(Oe.makedev(1, 3), { read: function() {
      return 0;
    }, write: function(e4, r3, t2, n3, o3) {
      return n3;
    } }), Oe.mkdev("/dev/null", Oe.makedev(1, 3)), We.register(Oe.makedev(5, 0), We.default_tty_ops), We.register(Oe.makedev(6, 0), We.default_tty1_ops), Oe.mkdev("/dev/tty", Oe.makedev(5, 0)), Oe.mkdev("/dev/tty1", Oe.makedev(6, 0));
    var e3 = xe();
    Oe.createDevice("/dev", "random", e3), Oe.createDevice("/dev", "urandom", e3), Oe.mkdir("/dev/shm"), Oe.mkdir("/dev/shm/tmp");
  }, createSpecialDirectories: function() {
    Oe.mkdir("/proc");
    var e3 = Oe.mkdir("/proc/self");
    Oe.mkdir("/proc/self/fd"), Oe.mount({ mount: function() {
      var r3 = Oe.createNode(e3, "fd", 16895, 73);
      return r3.node_ops = { lookup: function(e4, r4) {
        var t2 = +r4, n3 = Oe.getStream(t2);
        if (!n3)
          throw new Oe.ErrnoError(8);
        var o3 = { parent: null, mount: { mountpoint: "fake" }, node_ops: { readlink: function() {
          return n3.path;
        } } };
        return o3.parent = o3, o3;
      } }, r3;
    } }, {}, "/proc/self/fd");
  }, createStandardStreams: function() {
    o2.stdin ? Oe.createDevice("/dev", "stdin", o2.stdin) : Oe.symlink("/dev/tty", "/dev/stdin"), o2.stdout ? Oe.createDevice("/dev", "stdout", null, o2.stdout) : Oe.symlink("/dev/tty", "/dev/stdout"), o2.stderr ? Oe.createDevice("/dev", "stderr", null, o2.stderr) : Oe.symlink("/dev/tty1", "/dev/stderr"), Oe.open("/dev/stdin", 0), Oe.open("/dev/stdout", 1), Oe.open("/dev/stderr", 1);
  }, ensureErrnoError: function() {
    Oe.ErrnoError || (Oe.ErrnoError = function(e3, r3) {
      this.node = r3, this.setErrno = function(e4) {
        this.errno = e4;
      }, this.setErrno(e3), this.message = "FS error";
    }, Oe.ErrnoError.prototype = new Error(), Oe.ErrnoError.prototype.constructor = Oe.ErrnoError, [44].forEach(function(e3) {
      Oe.genericErrors[e3] = new Oe.ErrnoError(e3), Oe.genericErrors[e3].stack = "<generic error, no stack>";
    }));
  }, staticInit: function() {
    Oe.ensureErrnoError(), Oe.nameTable = new Array(4096), Oe.mount(Se, {}, "/"), Oe.createDefaultDirectories(), Oe.createDefaultDevices(), Oe.createSpecialDirectories(), Oe.filesystems = { MEMFS: Se };
  }, init: function(e3, r3, t2) {
    Oe.init.initialized = true, Oe.ensureErrnoError(), o2.stdin = e3 || o2.stdin, o2.stdout = r3 || o2.stdout, o2.stderr = t2 || o2.stderr, Oe.createStandardStreams();
  }, quit: function() {
    Oe.init.initialized = false;
    var e3 = o2._fflush;
    e3 && e3(0);
    for (var r3 = 0; r3 < Oe.streams.length; r3++) {
      var t2 = Oe.streams[r3];
      t2 && Oe.close(t2);
    }
  }, getMode: function(e3, r3) {
    var t2 = 0;
    return e3 && (t2 |= 365), r3 && (t2 |= 146), t2;
  }, findObject: function(e3, r3) {
    var t2 = Oe.analyzePath(e3, r3);
    return t2.exists ? t2.object : null;
  }, analyzePath: function(e3, r3) {
    try {
      e3 = (n3 = Oe.lookupPath(e3, { follow: !r3 })).path;
    } catch (o3) {
    }
    var t2 = { isRoot: false, exists: false, error: 0, name: null, path: null, object: null, parentExists: false, parentPath: null, parentObject: null };
    try {
      var n3 = Oe.lookupPath(e3, { parent: true });
      t2.parentExists = true, t2.parentPath = n3.path, t2.parentObject = n3.node, t2.name = Re.basename(e3), n3 = Oe.lookupPath(e3, { follow: !r3 }), t2.exists = true, t2.path = n3.path, t2.object = n3.node, t2.name = n3.node.name, t2.isRoot = "/" === n3.path;
    } catch (o3) {
      t2.error = o3.errno;
    }
    return t2;
  }, createPath: function(e3, r3, t2, n3) {
    e3 = "string" == typeof e3 ? e3 : Oe.getPath(e3);
    for (var o3 = r3.split("/").reverse(); o3.length; ) {
      var a3 = o3.pop();
      if (a3) {
        var i2 = Re.join2(e3, a3);
        try {
          Oe.mkdir(i2);
        } catch (s2) {
        }
        e3 = i2;
      }
    }
    return i2;
  }, createFile: function(e3, r3, t2, n3, o3) {
    var a3 = Re.join2("string" == typeof e3 ? e3 : Oe.getPath(e3), r3), i2 = Oe.getMode(n3, o3);
    return Oe.create(a3, i2);
  }, createDataFile: function(e3, r3, t2, n3, o3, a3) {
    var i2 = r3 ? Re.join2("string" == typeof e3 ? e3 : Oe.getPath(e3), r3) : e3, s2 = Oe.getMode(n3, o3), u2 = Oe.create(i2, s2);
    if (t2) {
      if ("string" == typeof t2) {
        for (var l2 = new Array(t2.length), c2 = 0, f2 = t2.length; c2 < f2; ++c2)
          l2[c2] = t2.charCodeAt(c2);
        t2 = l2;
      }
      Oe.chmod(u2, 146 | s2);
      var d2 = Oe.open(u2, 577);
      Oe.write(d2, t2, 0, t2.length, 0, a3), Oe.close(d2), Oe.chmod(u2, s2);
    }
    return u2;
  }, createDevice: function(e3, r3, t2, n3) {
    var o3 = Re.join2("string" == typeof e3 ? e3 : Oe.getPath(e3), r3), a3 = Oe.getMode(!!t2, !!n3);
    Oe.createDevice.major || (Oe.createDevice.major = 64);
    var i2 = Oe.makedev(Oe.createDevice.major++, 0);
    return Oe.registerDevice(i2, { open: function(e4) {
      e4.seekable = false;
    }, close: function(e4) {
      n3 && n3.buffer && n3.buffer.length && n3(10);
    }, read: function(e4, r4, n4, o4, a4) {
      for (var i3 = 0, s2 = 0; s2 < o4; s2++) {
        var u2;
        try {
          u2 = t2();
        } catch (l2) {
          throw new Oe.ErrnoError(29);
        }
        if (void 0 === u2 && 0 === i3)
          throw new Oe.ErrnoError(6);
        if (null == u2)
          break;
        i3++, r4[n4 + s2] = u2;
      }
      return i3 && (e4.node.timestamp = Date.now()), i3;
    }, write: function(e4, r4, t3, o4, a4) {
      for (var i3 = 0; i3 < o4; i3++)
        try {
          n3(r4[t3 + i3]);
        } catch (s2) {
          throw new Oe.ErrnoError(29);
        }
      return o4 && (e4.node.timestamp = Date.now()), i3;
    } }), Oe.mkdev(o3, a3, i2);
  }, forceLoadFile: function(e3) {
    if (e3.isDevice || e3.isFolder || e3.link || e3.contents)
      return true;
    if ("undefined" != typeof XMLHttpRequest)
      throw new Error("Lazy loading should have been performed (contents set) in createLazyFile, but it was not. Lazy loading only works in web workers. Use --embed-file or --preload-file in emcc on the main thread.");
    if (!s)
      throw new Error("Cannot load without read() or XMLHttpRequest.");
    try {
      e3.contents = so(s(e3.url), true), e3.usedBytes = e3.contents.length;
    } catch (r3) {
      throw new Oe.ErrnoError(29);
    }
  }, createLazyFile: function(e3, r3, t2, n3, o3) {
    function a3() {
      this.lengthKnown = false, this.chunks = [];
    }
    if (a3.prototype.get = function(e4) {
      if (!(e4 > this.length - 1 || e4 < 0)) {
        var r4 = e4 % this.chunkSize, t3 = e4 / this.chunkSize | 0;
        return this.getter(t3)[r4];
      }
    }, a3.prototype.setDataGetter = function(e4) {
      this.getter = e4;
    }, a3.prototype.cacheLength = function() {
      var e4 = new XMLHttpRequest();
      if (e4.open("HEAD", t2, false), e4.send(null), !(e4.status >= 200 && e4.status < 300 || 304 === e4.status))
        throw new Error("Couldn't load " + t2 + ". Status: " + e4.status);
      var r4, n4 = Number(e4.getResponseHeader("Content-length")), o4 = (r4 = e4.getResponseHeader("Accept-Ranges")) && "bytes" === r4, a4 = (r4 = e4.getResponseHeader("Content-Encoding")) && "gzip" === r4, i3 = 1048576;
      o4 || (i3 = n4);
      var s3 = function(e5, r5) {
        if (e5 > r5)
          throw new Error("invalid range (" + e5 + ", " + r5 + ") or no bytes requested!");
        if (r5 > n4 - 1)
          throw new Error("only " + n4 + " bytes available! programmer error!");
        var o5 = new XMLHttpRequest();
        if (o5.open("GET", t2, false), n4 !== i3 && o5.setRequestHeader("Range", "bytes=" + e5 + "-" + r5), "undefined" != typeof Uint8Array && (o5.responseType = "arraybuffer"), o5.overrideMimeType && o5.overrideMimeType("text/plain; charset=x-user-defined"), o5.send(null), !(o5.status >= 200 && o5.status < 300 || 304 === o5.status))
          throw new Error("Couldn't load " + t2 + ". Status: " + o5.status);
        return void 0 !== o5.response ? new Uint8Array(o5.response || []) : so(o5.responseText || "", true);
      }, u3 = this;
      u3.setDataGetter(function(e5) {
        var r5 = e5 * i3, t3 = (e5 + 1) * i3 - 1;
        if (t3 = Math.min(t3, n4 - 1), void 0 === u3.chunks[e5] && (u3.chunks[e5] = s3(r5, t3)), void 0 === u3.chunks[e5])
          throw new Error("doXHR failed!");
        return u3.chunks[e5];
      }), !a4 && n4 || (i3 = n4 = 1, n4 = this.getter(0).length, i3 = n4, g("LazyFiles on gzip forces download of the whole file when length is accessed")), this._length = n4, this._chunkSize = i3, this.lengthKnown = true;
    }, "undefined" != typeof XMLHttpRequest)
      throw "Cannot do synchronous binary XHRs outside webworkers in modern browsers. Use --embed-file or --preload-file in emcc";
    var i2 = { isDevice: false, url: t2 }, s2 = Oe.createFile(e3, r3, i2, n3, o3);
    i2.contents ? s2.contents = i2.contents : i2.url && (s2.contents = null, s2.url = i2.url), Object.defineProperties(s2, { usedBytes: { get: function() {
      return this.contents.length;
    } } });
    var u2 = {};
    return Object.keys(s2.stream_ops).forEach(function(e4) {
      var r4 = s2.stream_ops[e4];
      u2[e4] = function() {
        return Oe.forceLoadFile(s2), r4.apply(null, arguments);
      };
    }), u2.read = function(e4, r4, t3, n4, o4) {
      Oe.forceLoadFile(s2);
      var a4 = e4.node.contents;
      if (o4 >= a4.length)
        return 0;
      var i3 = Math.min(a4.length - o4, n4);
      if (a4.slice)
        for (var u3 = 0; u3 < i3; u3++)
          r4[t3 + u3] = a4[o4 + u3];
      else
        for (u3 = 0; u3 < i3; u3++)
          r4[t3 + u3] = a4.get(o4 + u3);
      return i3;
    }, s2.stream_ops = u2, s2;
  }, createPreloadedFile: function(e3, r3, t2, n3, a3, i2, s2, u2, l2, c2) {
    Browser.init();
    var f2 = r3 ? De.resolve(Re.join2(e3, r3)) : e3;
    function d2(t3) {
      function d3(t4) {
        c2 && c2(), u2 || Oe.createDataFile(e3, r3, t4, n3, a3, l2), i2 && i2(), de();
      }
      var E2 = false;
      o2.preloadPlugins.forEach(function(e4) {
        E2 || e4.canHandle(f2) && (e4.handle(t3, f2, d3, function() {
          s2 && s2(), de();
        }), E2 = true);
      }), E2 || d3(t3);
    }
    fe(), "string" == typeof t2 ? qe(t2, function(e4) {
      d2(e4);
    }, s2) : d2(t2);
  }, indexedDB: function() {
    return window.indexedDB || window.mozIndexedDB || window.webkitIndexedDB || window.msIndexedDB;
  }, DB_NAME: function() {
    return "EM_FS_" + window.location.pathname;
  }, DB_VERSION: 20, DB_STORE_NAME: "FILE_DATA", saveFilesToDB: function(e3, r3, t2) {
    r3 = r3 || function() {
    }, t2 = t2 || function() {
    };
    var n3 = Oe.indexedDB();
    try {
      var o3 = n3.open(Oe.DB_NAME(), Oe.DB_VERSION);
    } catch (a3) {
      return t2(a3);
    }
    o3.onupgradeneeded = function() {
      g("creating db"), o3.result.createObjectStore(Oe.DB_STORE_NAME);
    }, o3.onsuccess = function() {
      var n4 = o3.result.transaction([Oe.DB_STORE_NAME], "readwrite"), a3 = n4.objectStore(Oe.DB_STORE_NAME), i2 = 0, s2 = 0, u2 = e3.length;
      function l2() {
        0 == s2 ? r3() : t2();
      }
      e3.forEach(function(e4) {
        var r4 = a3.put(Oe.analyzePath(e4).object.contents, e4);
        r4.onsuccess = function() {
          ++i2 + s2 == u2 && l2();
        }, r4.onerror = function() {
          s2++, i2 + s2 == u2 && l2();
        };
      }), n4.onerror = t2;
    }, o3.onerror = t2;
  }, loadFilesFromDB: function(e3, r3, t2) {
    r3 = r3 || function() {
    }, t2 = t2 || function() {
    };
    var n3 = Oe.indexedDB();
    try {
      var o3 = n3.open(Oe.DB_NAME(), Oe.DB_VERSION);
    } catch (a3) {
      return t2(a3);
    }
    o3.onupgradeneeded = t2, o3.onsuccess = function() {
      var n4 = o3.result;
      try {
        var i2 = n4.transaction([Oe.DB_STORE_NAME], "readonly");
      } catch (a3) {
        return void t2(a3);
      }
      var s2 = i2.objectStore(Oe.DB_STORE_NAME), u2 = 0, l2 = 0, c2 = e3.length;
      function f2() {
        0 == l2 ? r3() : t2();
      }
      e3.forEach(function(e4) {
        var r4 = s2.get(e4);
        r4.onsuccess = function() {
          Oe.analyzePath(e4).exists && Oe.unlink(e4), Oe.createDataFile(Re.dirname(e4), Re.basename(e4), r4.result, true, true, true), ++u2 + l2 == c2 && f2();
        }, r4.onerror = function() {
          l2++, u2 + l2 == c2 && f2();
        };
      }), i2.onerror = t2;
    }, o3.onerror = t2;
  } }, Pe = { mappings: {}, DEFAULT_POLLMASK: 5, umask: 511, calculateAt: function(e3, r3, t2) {
    if ("/" === r3[0])
      return r3;
    var n3;
    if (-100 === e3)
      n3 = Oe.cwd();
    else {
      var o3 = Oe.getStream(e3);
      if (!o3)
        throw new Oe.ErrnoError(8);
      n3 = o3.path;
    }
    if (0 == r3.length) {
      if (!t2)
        throw new Oe.ErrnoError(44);
      return n3;
    }
    return Re.join2(n3, r3);
  }, doStat: function(e3, r3, t2) {
    try {
      var n3 = e3(r3);
    } catch (o3) {
      if (o3 && o3.node && Re.normalize(r3) !== Re.normalize(Oe.getPath(o3.node)))
        return -54;
      throw o3;
    }
    return O[t2 >> 2] = n3.dev, O[t2 + 4 >> 2] = 0, O[t2 + 8 >> 2] = n3.ino, O[t2 + 12 >> 2] = n3.mode, O[t2 + 16 >> 2] = n3.nlink, O[t2 + 20 >> 2] = n3.uid, O[t2 + 24 >> 2] = n3.gid, O[t2 + 28 >> 2] = n3.rdev, O[t2 + 32 >> 2] = 0, he = [n3.size >>> 0, (pe = n3.size, +Math.abs(pe) >= 1 ? pe > 0 ? (0 | Math.min(+Math.floor(pe / 4294967296), 4294967295)) >>> 0 : ~~+Math.ceil((pe - +(~~pe >>> 0)) / 4294967296) >>> 0 : 0)], O[t2 + 40 >> 2] = he[0], O[t2 + 44 >> 2] = he[1], O[t2 + 48 >> 2] = 4096, O[t2 + 52 >> 2] = n3.blocks, O[t2 + 56 >> 2] = n3.atime.getTime() / 1e3 | 0, O[t2 + 60 >> 2] = 0, O[t2 + 64 >> 2] = n3.mtime.getTime() / 1e3 | 0, O[t2 + 68 >> 2] = 0, O[t2 + 72 >> 2] = n3.ctime.getTime() / 1e3 | 0, O[t2 + 76 >> 2] = 0, he = [n3.ino >>> 0, (pe = n3.ino, +Math.abs(pe) >= 1 ? pe > 0 ? (0 | Math.min(+Math.floor(pe / 4294967296), 4294967295)) >>> 0 : ~~+Math.ceil((pe - +(~~pe >>> 0)) / 4294967296) >>> 0 : 0)], O[t2 + 80 >> 2] = he[0], O[t2 + 84 >> 2] = he[1], 0;
  }, doMsync: function(e3, r3, t2, n3, o3) {
    var a3 = b.slice(e3, e3 + t2);
    Oe.msync(r3, a3, o3, t2, n3);
  }, doMkdir: function(e3, r3) {
    return "/" === (e3 = Re.normalize(e3))[e3.length - 1] && (e3 = e3.substr(0, e3.length - 1)), Oe.mkdir(e3, r3, 0), 0;
  }, doMknod: function(e3, r3, t2) {
    switch (61440 & r3) {
      case 32768:
      case 8192:
      case 24576:
      case 4096:
      case 49152:
        break;
      default:
        return -28;
    }
    return Oe.mknod(e3, r3, t2), 0;
  }, doReadlink: function(e3, r3, t2) {
    if (t2 <= 0)
      return -28;
    var n3 = Oe.readlink(e3), o3 = Math.min(t2, x(n3)), a3 = W[r3 + o3];
    return R(n3, r3, t2 + 1), W[r3 + o3] = a3, o3;
  }, doAccess: function(e3, r3) {
    if (-8 & r3)
      return -28;
    var t2;
    if (!(t2 = Oe.lookupPath(e3, { follow: true }).node))
      return -44;
    var n3 = "";
    return 4 & r3 && (n3 += "r"), 2 & r3 && (n3 += "w"), 1 & r3 && (n3 += "x"), n3 && Oe.nodePermissions(t2, n3) ? -2 : 0;
  }, doDup: function(e3, r3, t2) {
    var n3 = Oe.getStream(t2);
    return n3 && Oe.close(n3), Oe.open(e3, r3, 0, t2, t2).fd;
  }, doReadv: function(e3, r3, t2, n3) {
    for (var o3 = 0, a3 = 0; a3 < t2; a3++) {
      var i2 = O[r3 + 8 * a3 >> 2], s2 = O[r3 + (8 * a3 + 4) >> 2], u2 = Oe.read(e3, W, i2, s2, n3);
      if (u2 < 0)
        return -1;
      if (o3 += u2, u2 < s2)
        break;
    }
    return o3;
  }, doWritev: function(e3, r3, t2, n3) {
    for (var o3 = 0, a3 = 0; a3 < t2; a3++) {
      var i2 = O[r3 + 8 * a3 >> 2], s2 = O[r3 + (8 * a3 + 4) >> 2], u2 = Oe.write(e3, W, i2, s2, n3);
      if (u2 < 0)
        return -1;
      o3 += u2;
    }
    return o3;
  }, varargs: void 0, get: function() {
    return Pe.varargs += 4, O[Pe.varargs - 4 >> 2];
  }, getStr: function(e3) {
    return H(e3);
  }, getStreamFromFD: function(e3) {
    var r3 = Oe.getStream(e3);
    if (!r3)
      throw new Oe.ErrnoError(8);
    return r3;
  }, get64: function(e3, r3) {
    return e3;
  } };
  function Le(e3, r3, t2) {
    Pe.varargs = t2;
    try {
      var n3 = Pe.getStreamFromFD(e3);
      switch (r3) {
        case 0:
          return (o3 = Pe.get()) < 0 ? -28 : Oe.open(n3.path, n3.flags, 0, o3).fd;
        case 1:
        case 2:
        case 13:
        case 14:
          return 0;
        case 3:
          return n3.flags;
        case 4:
          var o3 = Pe.get();
          return n3.flags |= o3, 0;
        case 12:
          return o3 = Pe.get(), S[o3 + 0 >> 1] = 2, 0;
        case 16:
        case 8:
        default:
          return -28;
        case 9:
          return Ue(28), -1;
      }
    } catch (a3) {
      return void 0 !== Oe && a3 instanceof Oe.ErrnoError || Ee(a3), -a3.errno;
    }
  }
  function Ze(e3, r3, t2) {
    Pe.varargs = t2;
    try {
      var n3 = Pe.getStreamFromFD(e3);
      switch (r3) {
        case 21509:
        case 21505:
        case 21510:
        case 21511:
        case 21512:
        case 21506:
        case 21507:
        case 21508:
        case 21523:
        case 21524:
          return n3.tty ? 0 : -59;
        case 21519:
          if (!n3.tty)
            return -59;
          var o3 = Pe.get();
          return O[o3 >> 2] = 0, 0;
        case 21520:
          return n3.tty ? -28 : -59;
        case 21531:
          return o3 = Pe.get(), Oe.ioctl(n3, r3, o3);
        default:
          Ee("bad ioctl syscall " + r3);
      }
    } catch (a3) {
      return void 0 !== Oe && a3 instanceof Oe.ErrnoError || Ee(a3), -a3.errno;
    }
  }
  function Ve(e3, r3, t2) {
    Pe.varargs = t2;
    try {
      var n3 = Pe.getStr(e3), o3 = t2 ? Pe.get() : 0;
      return Oe.open(n3, r3, o3).fd;
    } catch (a3) {
      return void 0 !== Oe && a3 instanceof Oe.ErrnoError || Ee(a3), -a3.errno;
    }
  }
  var Ne = {};
  function Ie(e3) {
    for (; e3.length; ) {
      var r3 = e3.pop();
      e3.pop()(r3);
    }
  }
  function Me(e3) {
    return this.fromWireType(P[e3 >> 2]);
  }
  var Ge = {}, Xe = {}, Je = {}, Ye = 48, Ke = 57;
  function ze(e3) {
    if (void 0 === e3)
      return "_unknown";
    var r3 = (e3 = e3.replace(/[^a-zA-Z0-9_]/g, "$")).charCodeAt(0);
    return r3 >= Ye && r3 <= Ke ? "_" + e3 : e3;
  }
  function je(e3, r3) {
    return e3 = ze(e3), function() {
      return r3.apply(this, arguments);
    };
  }
  function _e(e3, r3) {
    var t2 = je(r3, function(e4) {
      this.name = r3, this.message = e4;
      var t3 = new Error(e4).stack;
      void 0 !== t3 && (this.stack = this.toString() + "\n" + t3.replace(/^Error(:[^\n]*)?\n/, ""));
    });
    return t2.prototype = Object.create(e3.prototype), t2.prototype.constructor = t2, t2.prototype.toString = function() {
      return void 0 === this.message ? this.name : this.name + ": " + this.message;
    }, t2;
  }
  var $e = void 0;
  function er(e3) {
    throw new $e(e3);
  }
  function rr(e3, r3, t2) {
    function n3(r4) {
      var n4 = t2(r4);
      n4.length !== e3.length && er("Mismatched type converter count");
      for (var o4 = 0; o4 < e3.length; ++o4)
        cr(e3[o4], n4[o4]);
    }
    e3.forEach(function(e4) {
      Je[e4] = r3;
    });
    var o3 = new Array(r3.length), a3 = [], i2 = 0;
    r3.forEach(function(e4, r4) {
      Xe.hasOwnProperty(e4) ? o3[r4] = Xe[e4] : (a3.push(e4), Ge.hasOwnProperty(e4) || (Ge[e4] = []), Ge[e4].push(function() {
        o3[r4] = Xe[e4], ++i2 === a3.length && n3(o3);
      }));
    }), 0 === a3.length && n3(o3);
  }
  function tr(e3) {
    var r3 = Ne[e3];
    delete Ne[e3];
    var t2 = r3.rawConstructor, n3 = r3.rawDestructor, o3 = r3.fields;
    rr([e3], o3.map(function(e4) {
      return e4.getterReturnType;
    }).concat(o3.map(function(e4) {
      return e4.setterArgumentType;
    })), function(e4) {
      var a3 = {};
      return o3.forEach(function(r4, t3) {
        var n4 = r4.fieldName, i2 = e4[t3], s2 = r4.getter, u2 = r4.getterContext, l2 = e4[t3 + o3.length], c2 = r4.setter, f2 = r4.setterContext;
        a3[n4] = { read: function(e5) {
          return i2.fromWireType(s2(u2, e5));
        }, write: function(e5, r5) {
          var t4 = [];
          c2(f2, e5, l2.toWireType(t4, r5)), Ie(t4);
        } };
      }), [{ name: r3.name, fromWireType: function(e5) {
        var r4 = {};
        for (var t3 in a3)
          r4[t3] = a3[t3].read(e5);
        return n3(e5), r4;
      }, toWireType: function(e5, r4) {
        for (var o4 in a3)
          if (!(o4 in r4))
            throw new TypeError('Missing field:  "' + o4 + '"');
        var i2 = t2();
        for (o4 in a3)
          a3[o4].write(i2, r4[o4]);
        return null !== e5 && e5.push(n3, i2), i2;
      }, argPackAdvance: 8, readValueFromPointer: Me, destructorFunction: n3 }];
    });
  }
  function nr(e3, r3, t2, n3, o3) {
  }
  function or(e3) {
    switch (e3) {
      case 1:
        return 0;
      case 2:
        return 1;
      case 4:
        return 2;
      case 8:
        return 3;
      default:
        throw new TypeError("Unknown type size: " + e3);
    }
  }
  function ar() {
    for (var e3 = new Array(256), r3 = 0; r3 < 256; ++r3)
      e3[r3] = String.fromCharCode(r3);
    ir = e3;
  }
  var ir = void 0;
  function sr(e3) {
    for (var r3 = "", t2 = e3; b[t2]; )
      r3 += ir[b[t2++]];
    return r3;
  }
  var ur = void 0;
  function lr(e3) {
    throw new ur(e3);
  }
  function cr(e3, r3, t2) {
    if (t2 = t2 || {}, !("argPackAdvance" in r3))
      throw new TypeError("registerType registeredInstance requires argPackAdvance");
    var n3 = r3.name;
    if (e3 || lr('type "' + n3 + '" must have a positive integer typeid pointer'), Xe.hasOwnProperty(e3)) {
      if (t2.ignoreDuplicateRegistrations)
        return;
      lr("Cannot register type '" + n3 + "' twice");
    }
    if (Xe[e3] = r3, delete Je[e3], Ge.hasOwnProperty(e3)) {
      var o3 = Ge[e3];
      delete Ge[e3], o3.forEach(function(e4) {
        e4();
      });
    }
  }
  function fr(e3, r3, t2, n3, o3) {
    var a3 = or(t2);
    cr(e3, { name: r3 = sr(r3), fromWireType: function(e4) {
      return !!e4;
    }, toWireType: function(e4, r4) {
      return r4 ? n3 : o3;
    }, argPackAdvance: 8, readValueFromPointer: function(e4) {
      var n4;
      if (1 === t2)
        n4 = W;
      else if (2 === t2)
        n4 = S;
      else {
        if (4 !== t2)
          throw new TypeError("Unknown boolean type size: " + r3);
        n4 = O;
      }
      return this.fromWireType(n4[e4 >> a3]);
    }, destructorFunction: null });
  }
  var dr = [], Er = [{}, { value: void 0 }, { value: null }, { value: true }, { value: false }];
  function gr(e3) {
    e3 > 4 && 0 == --Er[e3].refcount && (Er[e3] = void 0, dr.push(e3));
  }
  function pr() {
    for (var e3 = 0, r3 = 5; r3 < Er.length; ++r3)
      void 0 !== Er[r3] && ++e3;
    return e3;
  }
  function hr() {
    for (var e3 = 5; e3 < Er.length; ++e3)
      if (void 0 !== Er[e3])
        return Er[e3];
    return null;
  }
  function mr() {
    o2.count_emval_handles = pr, o2.get_first_emval = hr;
  }
  function vr(e3) {
    switch (e3) {
      case void 0:
        return 1;
      case null:
        return 2;
      case true:
        return 3;
      case false:
        return 4;
      default:
        var r3 = dr.length ? dr.pop() : Er.length;
        return Er[r3] = { refcount: 1, value: e3 }, r3;
    }
  }
  function wr(e3, r3) {
    cr(e3, { name: r3 = sr(r3), fromWireType: function(e4) {
      var r4 = Er[e4].value;
      return gr(e4), r4;
    }, toWireType: function(e4, r4) {
      return vr(r4);
    }, argPackAdvance: 8, readValueFromPointer: Me, destructorFunction: null });
  }
  function yr(e3, r3, t2) {
    if (void 0 === e3[r3].overloadTable) {
      var n3 = e3[r3];
      e3[r3] = function() {
        return e3[r3].overloadTable.hasOwnProperty(arguments.length) || lr("Function '" + t2 + "' called with an invalid number of arguments (" + arguments.length + ") - expects one of (" + e3[r3].overloadTable + ")!"), e3[r3].overloadTable[arguments.length].apply(this, arguments);
      }, e3[r3].overloadTable = [], e3[r3].overloadTable[n3.argCount] = n3;
    }
  }
  function kr(e3, r3, t2) {
    o2.hasOwnProperty(e3) ? ((void 0 === t2 || void 0 !== o2[e3].overloadTable && void 0 !== o2[e3].overloadTable[t2]) && lr("Cannot register public name '" + e3 + "' twice"), yr(o2, e3, e3), o2.hasOwnProperty(t2) && lr("Cannot register multiple overloads of a function with the same number of arguments (" + t2 + ")!"), o2[e3].overloadTable[t2] = r3) : (o2[e3] = r3, void 0 !== t2 && (o2[e3].numArguments = t2));
  }
  function Br(e3, r3, t2) {
    switch (r3) {
      case 0:
        return function(e4) {
          var r4 = t2 ? W : b;
          return this.fromWireType(r4[e4]);
        };
      case 1:
        return function(e4) {
          var r4 = t2 ? S : q;
          return this.fromWireType(r4[e4 >> 1]);
        };
      case 2:
        return function(e4) {
          var r4 = t2 ? O : P;
          return this.fromWireType(r4[e4 >> 2]);
        };
      default:
        throw new TypeError("Unknown integer type: " + e3);
    }
  }
  function Qr(e3, r3, t2, n3) {
    var o3 = or(t2);
    function a3() {
    }
    r3 = sr(r3), a3.values = {}, cr(e3, { name: r3, constructor: a3, fromWireType: function(e4) {
      return this.constructor.values[e4];
    }, toWireType: function(e4, r4) {
      return r4.value;
    }, argPackAdvance: 8, readValueFromPointer: Br(r3, o3, n3), destructorFunction: null }), kr(r3, a3);
  }
  function Cr(e3) {
    var r3 = Eo(e3), t2 = sr(r3);
    return co(r3), t2;
  }
  function Ar(e3, r3) {
    var t2 = Xe[e3];
    return void 0 === t2 && lr(r3 + " has unknown type " + Cr(e3)), t2;
  }
  function Tr(e3, r3, t2) {
    var n3 = Ar(e3, "enum");
    r3 = sr(r3);
    var o3 = n3.constructor, a3 = Object.create(n3.constructor.prototype, { value: { value: t2 }, constructor: { value: je(n3.name + "_" + r3, function() {
    }) } });
    o3.values[t2] = a3, o3[r3] = a3;
  }
  function Fr(e3) {
    if (null === e3)
      return "null";
    var r3 = typeof e3;
    return "object" === r3 || "array" === r3 || "function" === r3 ? e3.toString() : "" + e3;
  }
  function Hr(e3, r3) {
    switch (r3) {
      case 2:
        return function(e4) {
          return this.fromWireType(L[e4 >> 2]);
        };
      case 3:
        return function(e4) {
          return this.fromWireType(Z[e4 >> 3]);
        };
      default:
        throw new TypeError("Unknown float type: " + e3);
    }
  }
  function Ur(e3, r3, t2) {
    var n3 = or(t2);
    cr(e3, { name: r3 = sr(r3), fromWireType: function(e4) {
      return e4;
    }, toWireType: function(e4, r4) {
      if ("number" != typeof r4 && "boolean" != typeof r4)
        throw new TypeError('Cannot convert "' + Fr(r4) + '" to ' + this.name);
      return r4;
    }, argPackAdvance: 8, readValueFromPointer: Hr(r3, n3), destructorFunction: null });
  }
  function Rr(e3, r3, t2, n3, o3) {
    var a3 = r3.length;
    a3 < 2 && lr("argTypes array size mismatch! Must at least get return value and 'this' types!");
    for (var i2 = null !== r3[1] && null !== t2, s2 = false, u2 = 1; u2 < r3.length; ++u2)
      if (null !== r3[u2] && void 0 === r3[u2].destructorFunction) {
        s2 = true;
        break;
      }
    var l2 = "void" !== r3[0].name, c2 = a3 - 2, f2 = new Array(c2), d2 = [], E2 = [];
    return function() {
      var t3;
      arguments.length !== c2 && lr("function " + e3 + " called with " + arguments.length + " arguments, expected " + c2 + " args!"), E2.length = 0, d2.length = i2 ? 2 : 1, d2[0] = o3, i2 && (t3 = r3[1].toWireType(E2, this), d2[1] = t3);
      for (var a4 = 0; a4 < c2; ++a4)
        f2[a4] = r3[a4 + 2].toWireType(E2, arguments[a4]), d2.push(f2[a4]);
      function u3(e4) {
        if (s2)
          Ie(E2);
        else
          for (var n4 = i2 ? 1 : 2; n4 < r3.length; n4++) {
            var o4 = 1 === n4 ? t3 : f2[n4 - 2];
            null !== r3[n4].destructorFunction && r3[n4].destructorFunction(o4);
          }
        if (l2)
          return r3[0].fromWireType(e4);
      }
      return u3(n3.apply(null, d2));
    };
  }
  function xr(e3, r3) {
    for (var t2 = [], n3 = 0; n3 < e3; n3++)
      t2.push(O[(r3 >> 2) + n3]);
    return t2;
  }
  function Dr(e3, r3, t2) {
    o2.hasOwnProperty(e3) || er("Replacing nonexistant public symbol"), void 0 !== o2[e3].overloadTable && void 0 !== t2 ? o2[e3].overloadTable[t2] = r3 : (o2[e3] = r3, o2[e3].argCount = t2);
  }
  function Wr(e3, r3, t2) {
    var n3 = o2["dynCall_" + e3];
    return t2 && t2.length ? n3.apply(null, [r3].concat(t2)) : n3.call(null, r3);
  }
  function br(e3, r3, t2) {
    return e3.includes("j") ? Wr(e3, r3, t2) : V.get(r3).apply(null, t2);
  }
  function Sr(e3, r3) {
    var t2 = [];
    return function() {
      t2.length = arguments.length;
      for (var n3 = 0; n3 < arguments.length; n3++)
        t2[n3] = arguments[n3];
      return br(e3, r3, t2);
    };
  }
  function qr(e3, r3) {
    function t2() {
      return e3.includes("j") ? Sr(e3, r3) : V.get(r3);
    }
    e3 = sr(e3);
    var n3 = t2();
    return "function" != typeof n3 && lr("unknown function pointer with signature " + e3 + ": " + r3), n3;
  }
  var Or = void 0;
  function Pr(e3, r3) {
    var t2 = [], n3 = {};
    function o3(e4) {
      n3[e4] || Xe[e4] || (Je[e4] ? Je[e4].forEach(o3) : (t2.push(e4), n3[e4] = true));
    }
    throw r3.forEach(o3), new Or(e3 + ": " + t2.map(Cr).join([", "]));
  }
  function Lr(e3, r3, t2, n3, o3, a3) {
    var i2 = xr(r3, t2);
    e3 = sr(e3), o3 = qr(n3, o3), kr(e3, function() {
      Pr("Cannot call " + e3 + " due to unbound types", i2);
    }, r3 - 1), rr([], i2, function(t3) {
      var n4 = [t3[0], null].concat(t3.slice(1));
      return Dr(e3, Rr(e3, n4, null, o3, a3), r3 - 1), [];
    });
  }
  function Zr(e3, r3, t2) {
    switch (r3) {
      case 0:
        return t2 ? function(e4) {
          return W[e4];
        } : function(e4) {
          return b[e4];
        };
      case 1:
        return t2 ? function(e4) {
          return S[e4 >> 1];
        } : function(e4) {
          return q[e4 >> 1];
        };
      case 2:
        return t2 ? function(e4) {
          return O[e4 >> 2];
        } : function(e4) {
          return P[e4 >> 2];
        };
      default:
        throw new TypeError("Unknown integer type: " + e3);
    }
  }
  function Vr(e3, r3, t2, n3, o3) {
    r3 = sr(r3), -1 === o3 && (o3 = 4294967295);
    var a3 = or(t2), i2 = function(e4) {
      return e4;
    };
    if (0 === n3) {
      var s2 = 32 - 8 * t2;
      i2 = function(e4) {
        return e4 << s2 >>> s2;
      };
    }
    var u2 = r3.includes("unsigned");
    cr(e3, { name: r3, fromWireType: i2, toWireType: function(e4, t3) {
      if ("number" != typeof t3 && "boolean" != typeof t3)
        throw new TypeError('Cannot convert "' + Fr(t3) + '" to ' + this.name);
      if (t3 < n3 || t3 > o3)
        throw new TypeError('Passing a number "' + Fr(t3) + '" from JS side to C/C++ side to an argument of type "' + r3 + '", which is outside the valid range [' + n3 + ", " + o3 + "]!");
      return u2 ? t3 >>> 0 : 0 | t3;
    }, argPackAdvance: 8, readValueFromPointer: Zr(r3, a3, 0 !== n3), destructorFunction: null });
  }
  function Nr(e3, r3, t2) {
    var n3 = [Int8Array, Uint8Array, Int16Array, Uint16Array, Int32Array, Uint32Array, Float32Array, Float64Array][r3];
    function o3(e4) {
      var r4 = P, t3 = r4[e4 >>= 2], o4 = r4[e4 + 1];
      return new n3(D, o4, t3);
    }
    cr(e3, { name: t2 = sr(t2), fromWireType: o3, argPackAdvance: 8, readValueFromPointer: o3 }, { ignoreDuplicateRegistrations: true });
  }
  function Ir(e3, r3) {
    var t2 = "std::string" === (r3 = sr(r3));
    cr(e3, { name: r3, fromWireType: function(e4) {
      var r4, n3 = P[e4 >> 2];
      if (t2)
        for (var o3 = e4 + 4, a3 = 0; a3 <= n3; ++a3) {
          var i2 = e4 + 4 + a3;
          if (a3 == n3 || 0 == b[i2]) {
            var s2 = H(o3, i2 - o3);
            void 0 === r4 ? r4 = s2 : (r4 += String.fromCharCode(0), r4 += s2), o3 = i2 + 1;
          }
        }
      else {
        var u2 = new Array(n3);
        for (a3 = 0; a3 < n3; ++a3)
          u2[a3] = String.fromCharCode(b[e4 + 4 + a3]);
        r4 = u2.join("");
      }
      return co(e4), r4;
    }, toWireType: function(e4, r4) {
      r4 instanceof ArrayBuffer && (r4 = new Uint8Array(r4));
      var n3 = "string" == typeof r4;
      n3 || r4 instanceof Uint8Array || r4 instanceof Uint8ClampedArray || r4 instanceof Int8Array || lr("Cannot pass non-string to std::string");
      var o3 = (t2 && n3 ? function() {
        return x(r4);
      } : function() {
        return r4.length;
      })(), a3 = fo(4 + o3 + 1);
      if (P[a3 >> 2] = o3, t2 && n3)
        R(r4, a3 + 4, o3 + 1);
      else if (n3)
        for (var i2 = 0; i2 < o3; ++i2) {
          var s2 = r4.charCodeAt(i2);
          s2 > 255 && (co(a3), lr("String has UTF-16 code units that do not fit in 8 bits")), b[a3 + 4 + i2] = s2;
        }
      else
        for (i2 = 0; i2 < o3; ++i2)
          b[a3 + 4 + i2] = r4[i2];
      return null !== e4 && e4.push(co, a3), a3;
    }, argPackAdvance: 8, readValueFromPointer: Me, destructorFunction: function(e4) {
      co(e4);
    } });
  }
  function Mr(e3, r3, t2) {
    var n3, o3, a3, i2, s2;
    t2 = sr(t2), 2 === r3 ? (n3 = I, o3 = M, i2 = G, a3 = function() {
      return q;
    }, s2 = 1) : 4 === r3 && (n3 = X, o3 = J, i2 = Y, a3 = function() {
      return P;
    }, s2 = 2), cr(e3, { name: t2, fromWireType: function(e4) {
      for (var t3, o4 = P[e4 >> 2], i3 = a3(), u2 = e4 + 4, l2 = 0; l2 <= o4; ++l2) {
        var c2 = e4 + 4 + l2 * r3;
        if (l2 == o4 || 0 == i3[c2 >> s2]) {
          var f2 = n3(u2, c2 - u2);
          void 0 === t3 ? t3 = f2 : (t3 += String.fromCharCode(0), t3 += f2), u2 = c2 + r3;
        }
      }
      return co(e4), t3;
    }, toWireType: function(e4, n4) {
      "string" != typeof n4 && lr("Cannot pass non-string to C++ string type " + t2);
      var a4 = i2(n4), u2 = fo(4 + a4 + r3);
      return P[u2 >> 2] = a4 >> s2, o3(n4, u2 + 4, a4 + r3), null !== e4 && e4.push(co, u2), u2;
    }, argPackAdvance: 8, readValueFromPointer: Me, destructorFunction: function(e4) {
      co(e4);
    } });
  }
  function Gr(e3, r3, t2, n3, o3, a3) {
    Ne[e3] = { name: sr(r3), rawConstructor: qr(t2, n3), rawDestructor: qr(o3, a3), fields: [] };
  }
  function Xr(e3, r3, t2, n3, o3, a3, i2, s2, u2, l2) {
    Ne[e3].fields.push({ fieldName: sr(r3), getterReturnType: t2, getter: qr(n3, o3), getterContext: a3, setterArgumentType: i2, setter: qr(s2, u2), setterContext: l2 });
  }
  function Jr(e3, r3) {
    cr(e3, { isVoid: true, name: r3 = sr(r3), argPackAdvance: 0, fromWireType: function() {
    }, toWireType: function(e4, r4) {
    } });
  }
  function Yr(e3) {
    e3 > 4 && (Er[e3].refcount += 1);
  }
  var Kr = {};
  function zr(e3) {
    var r3 = Kr[e3];
    return void 0 === r3 ? sr(e3) : r3;
  }
  function jr(e3) {
    return vr(zr(e3));
  }
  function _r() {
    return vr({});
  }
  function $r(e3) {
    return e3 || lr("Cannot use deleted val. handle = " + e3), Er[e3].value;
  }
  function et(e3, r3, t2) {
    e3 = $r(e3), r3 = $r(r3), t2 = $r(t2), e3[r3] = t2;
  }
  function rt(e3, r3) {
    return vr((e3 = Ar(e3, "_emval_take_value")).readValueFromPointer(r3));
  }
  function tt() {
    Ee("");
  }
  function nt(e3, r3, t2) {
    b.copyWithin(e3, r3, r3 + t2);
  }
  function ot(e3) {
    try {
      return w.grow(e3 - D.byteLength + 65535 >>> 16), _(w.buffer), 1;
    } catch (r3) {
    }
  }
  function at(e3) {
    var r3 = b.length, t2 = 2147483648;
    if ((e3 >>>= 0) > t2)
      return false;
    for (var n3 = 1; n3 <= 4; n3 *= 2) {
      var o3 = r3 * (1 + 0.2 / n3);
      if (o3 = Math.min(o3, e3 + 100663296), ot(Math.min(t2, j(Math.max(e3, o3), 65536))))
        return true;
    }
    return false;
  }
  function it(e3) {
    return !!(e3.dibvbi = e3.getExtension("WEBGL_draw_instanced_base_vertex_base_instance"));
  }
  function st(e3) {
    return !!(e3.mdibvbi = e3.getExtension("WEBGL_multi_draw_instanced_base_vertex_base_instance"));
  }
  function ut(e3) {
    return !!(e3.multiDrawWebgl = e3.getExtension("WEBGL_multi_draw"));
  }
  var lt = { counter: 1, buffers: [], programs: [], framebuffers: [], renderbuffers: [], textures: [], shaders: [], vaos: [], contexts: [], offscreenCanvases: {}, queries: [], samplers: [], transformFeedbacks: [], syncs: [], stringCache: {}, stringiCache: {}, unpackAlignment: 4, recordError: function(e3) {
    lt.lastError || (lt.lastError = e3);
  }, getNewId: function(e3) {
    for (var r3 = lt.counter++, t2 = e3.length; t2 < r3; t2++)
      e3[t2] = null;
    return r3;
  }, getSource: function(e3, r3, t2, n3) {
    for (var o3 = "", a3 = 0; a3 < r3; ++a3) {
      var i2 = n3 ? O[n3 + 4 * a3 >> 2] : -1;
      o3 += H(O[t2 + 4 * a3 >> 2], i2 < 0 ? void 0 : i2);
    }
    return o3;
  }, createContext: function(e3, r3) {
    if (o2.preinitializedWebGLContext) {
      var t2 = o2.preinitializedWebGLContext;
      r3.majorVersion = "undefined" != typeof WebGL2RenderingContext && t2 instanceof WebGL2RenderingContext ? 2 : 1;
    } else
      e3.getContextSafariWebGL2Fixed || (e3.getContextSafariWebGL2Fixed = e3.getContext, e3.getContext = function(r4, t3) {
        var n3 = e3.getContextSafariWebGL2Fixed(r4, t3);
        return "webgl" == r4 == n3 instanceof WebGLRenderingContext ? n3 : null;
      }), t2 = e3.getContext("webgl2", r3);
    return t2 ? lt.registerContext(t2, r3) : 0;
  }, registerContext: function(e3, r3) {
    var t2 = lt.getNewId(lt.contexts), n3 = { handle: t2, attributes: r3, version: r3.majorVersion, GLctx: e3 };
    return e3.canvas && (e3.canvas.GLctxObject = n3), lt.contexts[t2] = n3, (void 0 === r3.enableExtensionsByDefault || r3.enableExtensionsByDefault) && lt.initExtensions(n3), t2;
  }, makeContextCurrent: function(e3) {
    return lt.currentContext = lt.contexts[e3], o2.ctx = to = lt.currentContext && lt.currentContext.GLctx, !(e3 && !to);
  }, getContext: function(e3) {
    return lt.contexts[e3];
  }, deleteContext: function(e3) {
    lt.currentContext === lt.contexts[e3] && (lt.currentContext = null), "object" == typeof ct && ct.removeAllHandlersOnTarget(lt.contexts[e3].GLctx.canvas), lt.contexts[e3] && lt.contexts[e3].GLctx.canvas && (lt.contexts[e3].GLctx.canvas.GLctxObject = void 0), lt.contexts[e3] = null;
  }, initExtensions: function(e3) {
    if (e3 || (e3 = lt.currentContext), !e3.initExtensionsDone) {
      e3.initExtensionsDone = true;
      var r3 = e3.GLctx;
      it(r3), st(r3), e3.version >= 2 && (r3.disjointTimerQueryExt = r3.getExtension("EXT_disjoint_timer_query_webgl2")), (e3.version < 2 || !r3.disjointTimerQueryExt) && (r3.disjointTimerQueryExt = r3.getExtension("EXT_disjoint_timer_query")), ut(r3), (r3.getSupportedExtensions() || []).forEach(function(e4) {
        e4.includes("lose_context") || e4.includes("debug") || r3.getExtension(e4);
      });
    }
  } }, ct = { inEventHandler: 0, removeAllEventListeners: function() {
    for (var e3 = ct.eventHandlers.length - 1; e3 >= 0; --e3)
      ct._removeHandler(e3);
    ct.eventHandlers = [], ct.deferredCalls = [];
  }, registerRemoveEventListeners: function() {
    ct.removeEventListenersRegistered || (ct.removeEventListenersRegistered = true);
  }, deferredCalls: [], deferCall: function(e3, r3, t2) {
    function n3(e4, r4) {
      if (e4.length != r4.length)
        return false;
      for (var t3 in e4)
        if (e4[t3] != r4[t3])
          return false;
      return true;
    }
    for (var o3 in ct.deferredCalls) {
      var a3 = ct.deferredCalls[o3];
      if (a3.targetFunction == e3 && n3(a3.argsList, t2))
        return;
    }
    ct.deferredCalls.push({ targetFunction: e3, precedence: r3, argsList: t2 }), ct.deferredCalls.sort(function(e4, r4) {
      return e4.precedence < r4.precedence;
    });
  }, removeDeferredCalls: function(e3) {
    for (var r3 = 0; r3 < ct.deferredCalls.length; ++r3)
      ct.deferredCalls[r3].targetFunction == e3 && (ct.deferredCalls.splice(r3, 1), --r3);
  }, canPerformEventHandlerRequests: function() {
    return ct.inEventHandler && ct.currentEventHandler.allowsDeferredCalls;
  }, runDeferredCalls: function() {
    if (ct.canPerformEventHandlerRequests())
      for (var e3 = 0; e3 < ct.deferredCalls.length; ++e3) {
        var r3 = ct.deferredCalls[e3];
        ct.deferredCalls.splice(e3, 1), --e3, r3.targetFunction.apply(null, r3.argsList);
      }
  }, eventHandlers: [], removeAllHandlersOnTarget: function(e3, r3) {
    for (var t2 = 0; t2 < ct.eventHandlers.length; ++t2)
      ct.eventHandlers[t2].target != e3 || r3 && r3 != ct.eventHandlers[t2].eventTypeString || ct._removeHandler(t2--);
  }, _removeHandler: function(e3) {
    var r3 = ct.eventHandlers[e3];
    r3.target.removeEventListener(r3.eventTypeString, r3.eventListenerFunc, r3.useCapture), ct.eventHandlers.splice(e3, 1);
  }, registerOrRemoveHandler: function(e3) {
    var r3 = function(r4) {
      ++ct.inEventHandler, ct.currentEventHandler = e3, ct.runDeferredCalls(), e3.handlerFunc(r4), ct.runDeferredCalls(), --ct.inEventHandler;
    };
    if (e3.callbackfunc)
      e3.eventListenerFunc = r3, e3.target.addEventListener(e3.eventTypeString, r3, e3.useCapture), ct.eventHandlers.push(e3), ct.registerRemoveEventListeners();
    else
      for (var t2 = 0; t2 < ct.eventHandlers.length; ++t2)
        ct.eventHandlers[t2].target == e3.target && ct.eventHandlers[t2].eventTypeString == e3.eventTypeString && ct._removeHandler(t2--);
  }, getNodeNameForTarget: function(e3) {
    return e3 ? e3 == window ? "#window" : e3 == screen ? "#screen" : e3 && e3.nodeName ? e3.nodeName : "" : "";
  }, fullscreenEnabled: function() {
    return document.fullscreenEnabled || document.webkitFullscreenEnabled;
  } }, ft = ["default", "low-power", "high-performance"];
  function dt(e3) {
    return e3 > 2 ? H(e3) : e3;
  }
  var Et = [0, document, window];
  function gt(e3) {
    return e3 = dt(e3), Et[e3] || document.querySelector(e3);
  }
  function pt(e3) {
    return gt(e3);
  }
  function ht(e3, r3) {
    var t2 = r3 >> 2, n3 = O[t2 + 6], o3 = { alpha: !!O[t2 + 0], depth: !!O[t2 + 1], stencil: !!O[t2 + 2], antialias: !!O[t2 + 3], premultipliedAlpha: !!O[t2 + 4], preserveDrawingBuffer: !!O[t2 + 5], powerPreference: ft[n3], failIfMajorPerformanceCaveat: !!O[t2 + 7], majorVersion: O[t2 + 8], minorVersion: O[t2 + 9], enableExtensionsByDefault: O[t2 + 10], explicitSwapControl: O[t2 + 11], proxyContextToMainThread: O[t2 + 12], renderViaOffscreenBackBuffer: O[t2 + 13] }, a3 = pt(e3);
    return a3 ? o3.explicitSwapControl ? 0 : lt.createContext(a3, o3) : 0;
  }
  function mt(e3, r3) {
    return ht(e3, r3);
  }
  function vt(e3) {
    for (var r3 = e3 >> 2, t2 = 0; t2 < 14; ++t2)
      O[r3 + t2] = 0;
    O[r3 + 0] = O[r3 + 1] = O[r3 + 3] = O[r3 + 4] = O[r3 + 8] = O[r3 + 10] = 1;
  }
  function wt(e3) {
    return lt.makeContextCurrent(e3) ? 0 : -5;
  }
  var yt = {};
  function kt() {
    return c || "./this.program";
  }
  function Bt() {
    if (!Bt.strings) {
      var e3 = { USER: "web_user", LOGNAME: "web_user", PATH: "/", PWD: "/", HOME: "/home/web_user", LANG: ("object" == typeof navigator && navigator.languages && navigator.languages[0] || "C").replace("-", "_") + ".UTF-8", _: kt() };
      for (var r3 in yt)
        void 0 === yt[r3] ? delete e3[r3] : e3[r3] = yt[r3];
      var t2 = [];
      for (var r3 in e3)
        t2.push(r3 + "=" + e3[r3]);
      Bt.strings = t2;
    }
    return Bt.strings;
  }
  function Qt(e3, r3) {
    var t2 = 0;
    return Bt().forEach(function(n3, o3) {
      var a3 = r3 + t2;
      O[e3 + 4 * o3 >> 2] = a3, z(n3, a3), t2 += n3.length + 1;
    }), 0;
  }
  function Ct(e3, r3) {
    var t2 = Bt();
    O[e3 >> 2] = t2.length;
    var n3 = 0;
    return t2.forEach(function(e4) {
      n3 += e4.length + 1;
    }), O[r3 >> 2] = n3, 0;
  }
  function At(e3) {
    try {
      var r3 = Pe.getStreamFromFD(e3);
      return Oe.close(r3), 0;
    } catch (t2) {
      return void 0 !== Oe && t2 instanceof Oe.ErrnoError || Ee(t2), t2.errno;
    }
  }
  function Tt(e3, r3, t2, n3) {
    try {
      var o3 = Pe.getStreamFromFD(e3), a3 = Pe.doReadv(o3, r3, t2);
      return O[n3 >> 2] = a3, 0;
    } catch (i2) {
      return void 0 !== Oe && i2 instanceof Oe.ErrnoError || Ee(i2), i2.errno;
    }
  }
  function Ft(e3, r3, t2, n3, o3) {
    try {
      var a3 = Pe.getStreamFromFD(e3), i2 = 4294967296 * t2 + (r3 >>> 0), s2 = 9007199254740992;
      return i2 <= -s2 || i2 >= s2 ? -61 : (Oe.llseek(a3, i2, n3), he = [a3.position >>> 0, (pe = a3.position, +Math.abs(pe) >= 1 ? pe > 0 ? (0 | Math.min(+Math.floor(pe / 4294967296), 4294967295)) >>> 0 : ~~+Math.ceil((pe - +(~~pe >>> 0)) / 4294967296) >>> 0 : 0)], O[o3 >> 2] = he[0], O[o3 + 4 >> 2] = he[1], a3.getdents && 0 === i2 && 0 === n3 && (a3.getdents = null), 0);
    } catch (u2) {
      return void 0 !== Oe && u2 instanceof Oe.ErrnoError || Ee(u2), u2.errno;
    }
  }
  function Ht(e3, r3, t2, n3) {
    try {
      var o3 = Pe.getStreamFromFD(e3), a3 = Pe.doWritev(o3, r3, t2);
      return O[n3 >> 2] = a3, 0;
    } catch (i2) {
      return void 0 !== Oe && i2 instanceof Oe.ErrnoError || Ee(i2), i2.errno;
    }
  }
  function Ut(e3) {
    to.activeTexture(e3);
  }
  function Rt(e3, r3) {
    to.attachShader(lt.programs[e3], lt.shaders[r3]);
  }
  function xt(e3, r3, t2) {
    to.bindAttribLocation(lt.programs[e3], r3, H(t2));
  }
  function Dt(e3, r3) {
    35051 == e3 ? to.currentPixelPackBufferBinding = r3 : 35052 == e3 && (to.currentPixelUnpackBufferBinding = r3), to.bindBuffer(e3, lt.buffers[r3]);
  }
  function Wt(e3, r3, t2) {
    to.bindBufferBase(e3, r3, lt.buffers[t2]);
  }
  function bt(e3, r3, t2, n3, o3) {
    to.bindBufferRange(e3, r3, lt.buffers[t2], n3, o3);
  }
  function St(e3, r3) {
    to.bindFramebuffer(e3, lt.framebuffers[r3]);
  }
  function qt(e3, r3) {
    to.bindTexture(e3, lt.textures[r3]);
  }
  function Ot(e3) {
    to.bindVertexArray(lt.vaos[e3]);
  }
  function Pt(e3, r3, t2, n3) {
    t2 ? to.bufferData(e3, b, n3, t2, r3) : to.bufferData(e3, r3, n3);
  }
  function Lt(e3, r3, t2, n3) {
    to.bufferSubData(e3, r3, b, n3, t2);
  }
  function Zt(e3) {
    to.clear(e3);
  }
  function Vt(e3, r3, t2, n3) {
    to.clearBufferfi(e3, r3, t2, n3);
  }
  function Nt(e3, r3, t2) {
    to.clearBufferfv(e3, r3, L, t2 >> 2);
  }
  function It(e3, r3, t2) {
    to.clearBufferiv(e3, r3, O, t2 >> 2);
  }
  function Mt(e3, r3, t2) {
    to.clearBufferuiv(e3, r3, P, t2 >> 2);
  }
  function Gt(e3, r3, t2, n3) {
    to.clearColor(e3, r3, t2, n3);
  }
  function Xt(e3) {
    to.clearDepth(e3);
  }
  function Jt(e3) {
    to.clearStencil(e3);
  }
  function Yt(e3) {
    to.compileShader(lt.shaders[e3]);
  }
  function Kt() {
    var e3 = lt.getNewId(lt.programs), r3 = to.createProgram();
    return r3.name = e3, r3.maxUniformLength = r3.maxAttributeLength = r3.maxUniformBlockNameLength = 0, r3.uniformIdCounter = 1, lt.programs[e3] = r3, e3;
  }
  function zt(e3) {
    var r3 = lt.getNewId(lt.shaders);
    return lt.shaders[r3] = to.createShader(e3), r3;
  }
  function jt(e3, r3) {
    for (var t2 = 0; t2 < e3; t2++) {
      var n3 = O[r3 + 4 * t2 >> 2], o3 = lt.buffers[n3];
      o3 && (to.deleteBuffer(o3), o3.name = 0, lt.buffers[n3] = null, n3 == to.currentPixelPackBufferBinding && (to.currentPixelPackBufferBinding = 0), n3 == to.currentPixelUnpackBufferBinding && (to.currentPixelUnpackBufferBinding = 0));
    }
  }
  function _t(e3, r3) {
    for (var t2 = 0; t2 < e3; ++t2) {
      var n3 = O[r3 + 4 * t2 >> 2], o3 = lt.framebuffers[n3];
      o3 && (to.deleteFramebuffer(o3), o3.name = 0, lt.framebuffers[n3] = null);
    }
  }
  function $t(e3) {
    if (e3) {
      var r3 = lt.programs[e3];
      r3 ? (to.deleteProgram(r3), r3.name = 0, lt.programs[e3] = null) : lt.recordError(1281);
    }
  }
  function en(e3, r3) {
    for (var t2 = 0; t2 < e3; t2++) {
      var n3 = O[r3 + 4 * t2 >> 2], o3 = lt.queries[n3];
      o3 && (to.deleteQuery(o3), lt.queries[n3] = null);
    }
  }
  function rn(e3) {
    if (e3) {
      var r3 = lt.shaders[e3];
      r3 ? (to.deleteShader(r3), lt.shaders[e3] = null) : lt.recordError(1281);
    }
  }
  function tn(e3, r3) {
    for (var t2 = 0; t2 < e3; t2++) {
      var n3 = O[r3 + 4 * t2 >> 2], o3 = lt.textures[n3];
      o3 && (to.deleteTexture(o3), o3.name = 0, lt.textures[n3] = null);
    }
  }
  function nn(e3, r3) {
    for (var t2 = 0; t2 < e3; t2++) {
      var n3 = O[r3 + 4 * t2 >> 2];
      to.deleteVertexArray(lt.vaos[n3]), lt.vaos[n3] = null;
    }
  }
  function on(e3, r3, t2) {
    to.drawArrays(e3, r3, t2);
  }
  var an = [];
  function sn(e3, r3) {
    for (var t2 = an[e3], n3 = 0; n3 < e3; n3++)
      t2[n3] = O[r3 + 4 * n3 >> 2];
    to.drawBuffers(t2);
  }
  function un(e3, r3, t2, n3) {
    to.drawElements(e3, r3, t2, n3);
  }
  function ln(e3, r3, t2, n3, o3) {
    to.drawElementsInstanced(e3, r3, t2, n3, o3);
  }
  function cn(e3) {
    to.enableVertexAttribArray(e3);
  }
  function fn(e3, r3, t2, n3, o3) {
    to.framebufferTexture2D(e3, r3, t2, lt.textures[n3], o3);
  }
  function dn(e3, r3, t2, n3) {
    for (var o3 = 0; o3 < e3; o3++) {
      var a3 = to[t2](), i2 = a3 && lt.getNewId(n3);
      a3 ? (a3.name = i2, n3[i2] = a3) : lt.recordError(1282), O[r3 + 4 * o3 >> 2] = i2;
    }
  }
  function En(e3, r3) {
    dn(e3, r3, "createBuffer", lt.buffers);
  }
  function gn(e3, r3) {
    dn(e3, r3, "createFramebuffer", lt.framebuffers);
  }
  function pn(e3, r3) {
    dn(e3, r3, "createQuery", lt.queries);
  }
  function hn(e3, r3) {
    dn(e3, r3, "createTexture", lt.textures);
  }
  function mn(e3, r3) {
    dn(e3, r3, "createVertexArray", lt.vaos);
  }
  function vn(e3) {
    to.generateMipmap(e3);
  }
  function wn(e3, r3) {
    P[e3 >> 2] = r3, P[e3 + 4 >> 2] = (r3 - P[e3 >> 2]) / 4294967296;
  }
  function yn(e3, r3, t2) {
    if (r3) {
      var n3 = void 0;
      switch (e3) {
        case 36346:
          n3 = 1;
          break;
        case 36344:
          return void (0 != t2 && 1 != t2 && lt.recordError(1280));
        case 34814:
        case 36345:
          n3 = 0;
          break;
        case 34466:
          var o3 = to.getParameter(34467);
          n3 = o3 ? o3.length : 0;
          break;
        case 33309:
          if (lt.currentContext.version < 2)
            return void lt.recordError(1282);
          n3 = 2 * (to.getSupportedExtensions() || []).length;
          break;
        case 33307:
        case 33308:
          if (lt.currentContext.version < 2)
            return void lt.recordError(1280);
          n3 = 33307 == e3 ? 3 : 0;
      }
      if (void 0 === n3) {
        var a3 = to.getParameter(e3);
        switch (typeof a3) {
          case "number":
            n3 = a3;
            break;
          case "boolean":
            n3 = a3 ? 1 : 0;
            break;
          case "string":
            return void lt.recordError(1280);
          case "object":
            if (null === a3)
              switch (e3) {
                case 34964:
                case 35725:
                case 34965:
                case 36006:
                case 36007:
                case 32873:
                case 34229:
                case 36662:
                case 36663:
                case 35053:
                case 35055:
                case 36010:
                case 35097:
                case 35869:
                case 32874:
                case 36389:
                case 35983:
                case 35368:
                case 34068:
                  n3 = 0;
                  break;
                default:
                  return void lt.recordError(1280);
              }
            else {
              if (a3 instanceof Float32Array || a3 instanceof Uint32Array || a3 instanceof Int32Array || a3 instanceof Array) {
                for (var i2 = 0; i2 < a3.length; ++i2)
                  switch (t2) {
                    case 0:
                      O[r3 + 4 * i2 >> 2] = a3[i2];
                      break;
                    case 2:
                      L[r3 + 4 * i2 >> 2] = a3[i2];
                      break;
                    case 4:
                      W[r3 + i2 >> 0] = a3[i2] ? 1 : 0;
                  }
                return;
              }
              try {
                n3 = 0 | a3.name;
              } catch (s2) {
                return lt.recordError(1280), void p("GL_INVALID_ENUM in glGet" + t2 + "v: Unknown object returned from WebGL getParameter(" + e3 + ")! (error: " + s2 + ")");
              }
            }
            break;
          default:
            return lt.recordError(1280), void p("GL_INVALID_ENUM in glGet" + t2 + "v: Native code calling glGet" + t2 + "v(" + e3 + ") and it returns " + a3 + " of type " + typeof a3 + "!");
        }
      }
      switch (t2) {
        case 1:
          wn(r3, n3);
          break;
        case 0:
          O[r3 >> 2] = n3;
          break;
        case 2:
          L[r3 >> 2] = n3;
          break;
        case 4:
          W[r3 >> 0] = n3 ? 1 : 0;
      }
    } else
      lt.recordError(1281);
  }
  function kn(e3, r3) {
    yn(e3, r3, 0);
  }
  function Bn(e3, r3, t2) {
    if (t2) {
      var n3, o3 = lt.queries[e3];
      wn(t2, "boolean" == typeof (n3 = lt.currentContext.version < 2 ? to.disjointTimerQueryExt.getQueryObjectEXT(o3, r3) : to.getQueryParameter(o3, r3)) ? n3 ? 1 : 0 : n3);
    } else
      lt.recordError(1281);
  }
  function Qn(e3, r3, t2) {
    if (t2) {
      var n3, o3 = lt.queries[e3], a3 = to.disjointTimerQueryExt.getQueryObjectEXT(o3, r3);
      n3 = "boolean" == typeof a3 ? a3 ? 1 : 0 : a3, O[t2 >> 2] = n3;
    } else
      lt.recordError(1281);
  }
  function Cn(e3, r3) {
    return to.getUniformBlockIndex(lt.programs[e3], H(r3));
  }
  function An(e3) {
    return parseInt(e3);
  }
  function Tn(e3) {
    return "]" == e3.slice(-1) && e3.lastIndexOf("[");
  }
  function Fn(e3) {
    var r3, t2, n3 = e3.uniformLocsById, o3 = e3.uniformSizeAndIdsByName;
    if (!n3)
      for (e3.uniformLocsById = n3 = {}, e3.uniformArrayNamesById = {}, r3 = 0; r3 < to.getProgramParameter(e3, 35718); ++r3) {
        var a3 = to.getActiveUniform(e3, r3), i2 = a3.name, s2 = a3.size, u2 = Tn(i2), l2 = u2 > 0 ? i2.slice(0, u2) : i2, c2 = e3.uniformIdCounter;
        for (e3.uniformIdCounter += s2, o3[l2] = [s2, c2], t2 = 0; t2 < s2; ++t2)
          n3[c2] = t2, e3.uniformArrayNamesById[c2++] = l2;
      }
  }
  function Hn(e3, r3) {
    if (r3 = H(r3), e3 = lt.programs[e3]) {
      Fn(e3);
      var t2 = e3.uniformLocsById, n3 = 0, o3 = r3, a3 = Tn(r3);
      a3 > 0 && (n3 = An(r3.slice(a3 + 1)) >>> 0, o3 = r3.slice(0, a3));
      var i2 = e3.uniformSizeAndIdsByName[o3];
      if (i2 && n3 < i2[0] && (t2[n3 += i2[1]] = t2[n3] || to.getUniformLocation(e3, r3)))
        return n3;
    } else
      lt.recordError(1281);
    return -1;
  }
  function Un(e3) {
    e3 = lt.programs[e3], to.linkProgram(e3), e3.uniformLocsById = 0, e3.uniformSizeAndIdsByName = {};
  }
  function Rn(e3, r3) {
    3317 == e3 && (lt.unpackAlignment = r3), to.pixelStorei(e3, r3);
  }
  function xn(e3, r3) {
    to.disjointTimerQueryExt.queryCounterEXT(lt.queries[e3], r3);
  }
  function Dn(e3) {
    to.readBuffer(e3);
  }
  function Wn(e3) {
    return 0 == (e3 -= 5120) ? W : 1 == e3 ? b : 2 == e3 ? S : 4 == e3 ? O : 6 == e3 ? L : 5 == e3 || 28922 == e3 || 28520 == e3 || 30779 == e3 || 30782 == e3 ? P : q;
  }
  function bn(e3) {
    return 31 - Math.clz32(e3.BYTES_PER_ELEMENT);
  }
  function Sn(e3, r3, t2, n3, o3, a3, i2) {
    if (to.currentPixelPackBufferBinding)
      to.readPixels(e3, r3, t2, n3, o3, a3, i2);
    else {
      var s2 = Wn(a3);
      to.readPixels(e3, r3, t2, n3, o3, a3, s2, i2 >> bn(s2));
    }
  }
  function qn(e3, r3, t2, n3) {
    var o3 = lt.getSource(e3, r3, t2, n3);
    to.shaderSource(lt.shaders[e3], o3);
  }
  function On(e3, r3, t2, n3, o3, a3, i2, s2, u2, l2) {
    if (to.currentPixelUnpackBufferBinding)
      to.texImage3D(e3, r3, t2, n3, o3, a3, i2, s2, u2, l2);
    else if (l2) {
      var c2 = Wn(u2);
      to.texImage3D(e3, r3, t2, n3, o3, a3, i2, s2, u2, c2, l2 >> bn(c2));
    } else
      to.texImage3D(e3, r3, t2, n3, o3, a3, i2, s2, u2, null);
  }
  function Pn(e3, r3, t2) {
    to.texParameteri(e3, r3, t2);
  }
  function Ln(e3, r3, t2, n3, o3) {
    to.texStorage2D(e3, r3, t2, n3, o3);
  }
  function Zn(e3, r3, t2, n3, o3, a3, i2, s2, u2) {
    if (to.currentPixelUnpackBufferBinding)
      to.texSubImage2D(e3, r3, t2, n3, o3, a3, i2, s2, u2);
    else if (u2) {
      var l2 = Wn(s2);
      to.texSubImage2D(e3, r3, t2, n3, o3, a3, i2, s2, l2, u2 >> bn(l2));
    } else
      to.texSubImage2D(e3, r3, t2, n3, o3, a3, i2, s2, null);
  }
  function Vn(e3, r3, t2, n3, o3, a3, i2, s2, u2, l2, c2) {
    if (to.currentPixelUnpackBufferBinding)
      to.texSubImage3D(e3, r3, t2, n3, o3, a3, i2, s2, u2, l2, c2);
    else if (c2) {
      var f2 = Wn(l2);
      to.texSubImage3D(e3, r3, t2, n3, o3, a3, i2, s2, u2, l2, f2, c2 >> bn(f2));
    } else
      to.texSubImage3D(e3, r3, t2, n3, o3, a3, i2, s2, u2, l2, null);
  }
  function Nn(e3) {
    var r3 = to.currentProgram;
    if (r3) {
      var t2 = r3.uniformLocsById[e3];
      return "number" == typeof t2 && (r3.uniformLocsById[e3] = t2 = to.getUniformLocation(r3, r3.uniformArrayNamesById[e3] + (t2 > 0 ? "[" + t2 + "]" : ""))), t2;
    }
    lt.recordError(1282);
  }
  function In(e3, r3) {
    to.uniform1i(Nn(e3), r3);
  }
  function Mn(e3, r3, t2) {
    e3 = lt.programs[e3], to.uniformBlockBinding(e3, r3, t2);
  }
  function Gn(e3) {
    e3 = lt.programs[e3], to.useProgram(e3), to.currentProgram = e3;
  }
  function Xn(e3, r3, t2, n3, o3) {
    to.vertexAttribIPointer(e3, r3, t2, n3, o3);
  }
  function Jn(e3, r3, t2, n3, o3, a3) {
    to.vertexAttribPointer(e3, r3, t2, !!n3, o3, a3);
  }
  function Yn(e3) {
  }
  function Kn(e3) {
    return e3 % 4 == 0 && (e3 % 100 != 0 || e3 % 400 == 0);
  }
  function zn(e3, r3) {
    for (var t2 = 0, n3 = 0; n3 <= r3; t2 += e3[n3++])
      ;
    return t2;
  }
  var jn = [31, 29, 31, 30, 31, 30, 31, 31, 30, 31, 30, 31], _n = [31, 28, 31, 30, 31, 30, 31, 31, 30, 31, 30, 31];
  function $n(e3, r3) {
    for (var t2 = new Date(e3.getTime()); r3 > 0; ) {
      var n3 = Kn(t2.getFullYear()), o3 = t2.getMonth(), a3 = (n3 ? jn : _n)[o3];
      if (!(r3 > a3 - t2.getDate()))
        return t2.setDate(t2.getDate() + r3), t2;
      r3 -= a3 - t2.getDate() + 1, t2.setDate(1), o3 < 11 ? t2.setMonth(o3 + 1) : (t2.setMonth(0), t2.setFullYear(t2.getFullYear() + 1));
    }
    return t2;
  }
  function eo(e3, r3, t2, n3) {
    var o3 = O[n3 + 40 >> 2], a3 = { tm_sec: O[n3 >> 2], tm_min: O[n3 + 4 >> 2], tm_hour: O[n3 + 8 >> 2], tm_mday: O[n3 + 12 >> 2], tm_mon: O[n3 + 16 >> 2], tm_year: O[n3 + 20 >> 2], tm_wday: O[n3 + 24 >> 2], tm_yday: O[n3 + 28 >> 2], tm_isdst: O[n3 + 32 >> 2], tm_gmtoff: O[n3 + 36 >> 2], tm_zone: o3 ? H(o3) : "" }, i2 = H(t2), s2 = { "%c": "%a %b %d %H:%M:%S %Y", "%D": "%m/%d/%y", "%F": "%Y-%m-%d", "%h": "%b", "%r": "%I:%M:%S %p", "%R": "%H:%M", "%T": "%H:%M:%S", "%x": "%m/%d/%y", "%X": "%H:%M:%S", "%Ec": "%c", "%EC": "%C", "%Ex": "%m/%d/%y", "%EX": "%H:%M:%S", "%Ey": "%y", "%EY": "%Y", "%Od": "%d", "%Oe": "%e", "%OH": "%H", "%OI": "%I", "%Om": "%m", "%OM": "%M", "%OS": "%S", "%Ou": "%u", "%OU": "%U", "%OV": "%V", "%Ow": "%w", "%OW": "%W", "%Oy": "%y" };
    for (var u2 in s2)
      i2 = i2.replace(new RegExp(u2, "g"), s2[u2]);
    var l2 = ["Sunday", "Monday", "Tuesday", "Wednesday", "Thursday", "Friday", "Saturday"], c2 = ["January", "February", "March", "April", "May", "June", "July", "August", "September", "October", "November", "December"];
    function f2(e4, r4, t3) {
      for (var n4 = "number" == typeof e4 ? e4.toString() : e4 || ""; n4.length < r4; )
        n4 = t3[0] + n4;
      return n4;
    }
    function d2(e4, r4) {
      return f2(e4, r4, "0");
    }
    function E2(e4, r4) {
      function t3(e5) {
        return e5 < 0 ? -1 : e5 > 0 ? 1 : 0;
      }
      var n4;
      return 0 === (n4 = t3(e4.getFullYear() - r4.getFullYear())) && 0 === (n4 = t3(e4.getMonth() - r4.getMonth())) && (n4 = t3(e4.getDate() - r4.getDate())), n4;
    }
    function g2(e4) {
      switch (e4.getDay()) {
        case 0:
          return new Date(e4.getFullYear() - 1, 11, 29);
        case 1:
          return e4;
        case 2:
          return new Date(e4.getFullYear(), 0, 3);
        case 3:
          return new Date(e4.getFullYear(), 0, 2);
        case 4:
          return new Date(e4.getFullYear(), 0, 1);
        case 5:
          return new Date(e4.getFullYear() - 1, 11, 31);
        case 6:
          return new Date(e4.getFullYear() - 1, 11, 30);
      }
    }
    function p2(e4) {
      var r4 = $n(new Date(e4.tm_year + 1900, 0, 1), e4.tm_yday), t3 = new Date(r4.getFullYear(), 0, 4), n4 = new Date(r4.getFullYear() + 1, 0, 4), o4 = g2(t3), a4 = g2(n4);
      return E2(o4, r4) <= 0 ? E2(a4, r4) <= 0 ? r4.getFullYear() + 1 : r4.getFullYear() : r4.getFullYear() - 1;
    }
    var h2 = { "%a": function(e4) {
      return l2[e4.tm_wday].substring(0, 3);
    }, "%A": function(e4) {
      return l2[e4.tm_wday];
    }, "%b": function(e4) {
      return c2[e4.tm_mon].substring(0, 3);
    }, "%B": function(e4) {
      return c2[e4.tm_mon];
    }, "%C": function(e4) {
      return d2((e4.tm_year + 1900) / 100 | 0, 2);
    }, "%d": function(e4) {
      return d2(e4.tm_mday, 2);
    }, "%e": function(e4) {
      return f2(e4.tm_mday, 2, " ");
    }, "%g": function(e4) {
      return p2(e4).toString().substring(2);
    }, "%G": function(e4) {
      return p2(e4);
    }, "%H": function(e4) {
      return d2(e4.tm_hour, 2);
    }, "%I": function(e4) {
      var r4 = e4.tm_hour;
      return 0 == r4 ? r4 = 12 : r4 > 12 && (r4 -= 12), d2(r4, 2);
    }, "%j": function(e4) {
      return d2(e4.tm_mday + zn(Kn(e4.tm_year + 1900) ? jn : _n, e4.tm_mon - 1), 3);
    }, "%m": function(e4) {
      return d2(e4.tm_mon + 1, 2);
    }, "%M": function(e4) {
      return d2(e4.tm_min, 2);
    }, "%n": function() {
      return "\n";
    }, "%p": function(e4) {
      return e4.tm_hour >= 0 && e4.tm_hour < 12 ? "AM" : "PM";
    }, "%S": function(e4) {
      return d2(e4.tm_sec, 2);
    }, "%t": function() {
      return "	";
    }, "%u": function(e4) {
      return e4.tm_wday || 7;
    }, "%U": function(e4) {
      var r4 = new Date(e4.tm_year + 1900, 0, 1), t3 = 0 === r4.getDay() ? r4 : $n(r4, 7 - r4.getDay()), n4 = new Date(e4.tm_year + 1900, e4.tm_mon, e4.tm_mday);
      if (E2(t3, n4) < 0) {
        var o4 = zn(Kn(n4.getFullYear()) ? jn : _n, n4.getMonth() - 1) - 31, a4 = 31 - t3.getDate() + o4 + n4.getDate();
        return d2(Math.ceil(a4 / 7), 2);
      }
      return 0 === E2(t3, r4) ? "01" : "00";
    }, "%V": function(e4) {
      var r4, t3 = new Date(e4.tm_year + 1900, 0, 4), n4 = new Date(e4.tm_year + 1901, 0, 4), o4 = g2(t3), a4 = g2(n4), i3 = $n(new Date(e4.tm_year + 1900, 0, 1), e4.tm_yday);
      return E2(i3, o4) < 0 ? "53" : E2(a4, i3) <= 0 ? "01" : (r4 = o4.getFullYear() < e4.tm_year + 1900 ? e4.tm_yday + 32 - o4.getDate() : e4.tm_yday + 1 - o4.getDate(), d2(Math.ceil(r4 / 7), 2));
    }, "%w": function(e4) {
      return e4.tm_wday;
    }, "%W": function(e4) {
      var r4 = new Date(e4.tm_year, 0, 1), t3 = 1 === r4.getDay() ? r4 : $n(r4, 0 === r4.getDay() ? 1 : 7 - r4.getDay() + 1), n4 = new Date(e4.tm_year + 1900, e4.tm_mon, e4.tm_mday);
      if (E2(t3, n4) < 0) {
        var o4 = zn(Kn(n4.getFullYear()) ? jn : _n, n4.getMonth() - 1) - 31, a4 = 31 - t3.getDate() + o4 + n4.getDate();
        return d2(Math.ceil(a4 / 7), 2);
      }
      return 0 === E2(t3, r4) ? "01" : "00";
    }, "%y": function(e4) {
      return (e4.tm_year + 1900).toString().substring(2);
    }, "%Y": function(e4) {
      return e4.tm_year + 1900;
    }, "%z": function(e4) {
      var r4 = e4.tm_gmtoff, t3 = r4 >= 0;
      return r4 = (r4 = Math.abs(r4) / 60) / 60 * 100 + r4 % 60, (t3 ? "+" : "-") + String("0000" + r4).slice(-4);
    }, "%Z": function(e4) {
      return e4.tm_zone;
    }, "%%": function() {
      return "%";
    } };
    for (var u2 in h2)
      i2.includes(u2) && (i2 = i2.replace(new RegExp(u2, "g"), h2[u2](a3)));
    var m2 = so(i2, false);
    return m2.length > r3 ? 0 : (K(m2, e3), m2.length - 1);
  }
  function ro(e3, r3, t2, n3) {
    return eo(e3, r3, t2, n3);
  }
  var to, no = function(e3, r3, t2, n3) {
    e3 || (e3 = this), this.parent = e3, this.mount = e3.mount, this.mounted = null, this.id = Oe.nextInode++, this.name = r3, this.mode = t2, this.node_ops = {}, this.stream_ops = {}, this.rdev = n3;
  }, oo = 365, ao = 146;
  Object.defineProperties(no.prototype, { read: { get: function() {
    return (this.mode & oo) === oo;
  }, set: function(e3) {
    e3 ? this.mode |= oo : this.mode &= ~oo;
  } }, write: { get: function() {
    return (this.mode & ao) === ao;
  }, set: function(e3) {
    e3 ? this.mode |= ao : this.mode &= ~ao;
  } }, isFolder: { get: function() {
    return Oe.isDir(this.mode);
  } }, isDevice: { get: function() {
    return Oe.isChrdev(this.mode);
  } } }), Oe.FSNode = no, Oe.staticInit(), o2.FS_createPath = Oe.createPath, o2.FS_createDataFile = Oe.createDataFile, o2.FS_createPreloadedFile = Oe.createPreloadedFile, o2.FS_createLazyFile = Oe.createLazyFile, o2.FS_createDevice = Oe.createDevice, o2.FS_unlink = Oe.unlink, $e = o2.InternalError = _e(Error, "InternalError"), ar(), ur = o2.BindingError = _e(Error, "BindingError"), mr(), Or = o2.UnboundTypeError = _e(Error, "UnboundTypeError");
  for (var io = 0; io < 32; ++io)
    an.push(new Array(io));
  function so(e3, r3, t2) {
    var n3 = t2 > 0 ? t2 : x(e3) + 1, o3 = new Array(n3), a3 = U(e3, o3, 0, o3.length);
    return r3 && (o3.length = a3), o3;
  }
  var uo = "function" == typeof atob ? atob : function(e3) {
    var r3, t2, n3, o3, a3, i2, s2 = "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/=", u2 = "", l2 = 0;
    e3 = e3.replace(/[^A-Za-z0-9\+\/\=]/g, "");
    do {
      r3 = s2.indexOf(e3.charAt(l2++)) << 2 | (o3 = s2.indexOf(e3.charAt(l2++))) >> 4, t2 = (15 & o3) << 4 | (a3 = s2.indexOf(e3.charAt(l2++))) >> 2, n3 = (3 & a3) << 6 | (i2 = s2.indexOf(e3.charAt(l2++))), u2 += String.fromCharCode(r3), 64 !== a3 && (u2 += String.fromCharCode(t2)), 64 !== i2 && (u2 += String.fromCharCode(n3));
    } while (l2 < e3.length);
    return u2;
  }, lo = { __assert_fail: Qe, __cxa_allocate_exception: Ce, __cxa_atexit: Te, __cxa_throw: He, __sys_fcntl64: Le, __sys_ioctl: Ze, __sys_open: Ve, _embind_finalize_value_object: tr, _embind_register_bigint: nr, _embind_register_bool: fr, _embind_register_emval: wr, _embind_register_enum: Qr, _embind_register_enum_value: Tr, _embind_register_float: Ur, _embind_register_function: Lr, _embind_register_integer: Vr, _embind_register_memory_view: Nr, _embind_register_std_string: Ir, _embind_register_std_wstring: Mr, _embind_register_value_object: Gr, _embind_register_value_object_field: Xr, _embind_register_void: Jr, _emval_decref: gr, _emval_incref: Yr, _emval_new_cstring: jr, _emval_new_object: _r, _emval_set_property: et, _emval_take_value: rt, abort: tt, emscripten_memcpy_big: nt, emscripten_resize_heap: at, emscripten_webgl_create_context: mt, emscripten_webgl_init_context_attributes: vt, emscripten_webgl_make_context_current: wt, environ_get: Qt, environ_sizes_get: Ct, fd_close: At, fd_read: Tt, fd_seek: Ft, fd_write: Ht, glActiveTexture: Ut, glAttachShader: Rt, glBindAttribLocation: xt, glBindBuffer: Dt, glBindBufferBase: Wt, glBindBufferRange: bt, glBindFramebuffer: St, glBindTexture: qt, glBindVertexArray: Ot, glBufferData: Pt, glBufferSubData: Lt, glClear: Zt, glClearBufferfi: Vt, glClearBufferfv: Nt, glClearBufferiv: It, glClearBufferuiv: Mt, glClearColor: Gt, glClearDepthf: Xt, glClearStencil: Jt, glCompileShader: Yt, glCreateProgram: Kt, glCreateShader: zt, glDeleteBuffers: jt, glDeleteFramebuffers: _t, glDeleteProgram: $t, glDeleteQueries: en, glDeleteShader: rn, glDeleteTextures: tn, glDeleteVertexArrays: nn, glDrawArrays: on, glDrawBuffers: sn, glDrawElements: un, glDrawElementsInstanced: ln, glEnableVertexAttribArray: cn, glFramebufferTexture2D: fn, glGenBuffers: En, glGenFramebuffers: gn, glGenQueries: pn, glGenTextures: hn, glGenVertexArrays: mn, glGenerateMipmap: vn, glGetIntegerv: kn, glGetQueryObjectui64vEXT: Bn, glGetQueryObjectuivEXT: Qn, glGetUniformBlockIndex: Cn, glGetUniformLocation: Hn, glLinkProgram: Un, glPixelStorei: Rn, glQueryCounterEXT: xn, glReadBuffer: Dn, glReadPixels: Sn, glShaderSource: qn, glTexImage3D: On, glTexParameteri: Pn, glTexStorage2D: Ln, glTexSubImage2D: Zn, glTexSubImage3D: Vn, glUniform1i: In, glUniformBlockBinding: Mn, glUseProgram: Gn, glVertexAttribIPointer: Xn, glVertexAttribPointer: Jn, setTempRet0: Yn, strftime_l: ro };
  ke(), o2.___wasm_call_ctors = function() {
    return (o2.___wasm_call_ctors = o2.asm.__wasm_call_ctors).apply(null, arguments);
  };
  var co = o2._free = function() {
    return (co = o2._free = o2.asm.free).apply(null, arguments);
  }, fo = o2._malloc = function() {
    return (fo = o2._malloc = o2.asm.malloc).apply(null, arguments);
  };
  o2.__Z16get_new_requestsi = function() {
    return (o2.__Z16get_new_requestsi = o2.asm._Z16get_new_requestsi).apply(null, arguments);
  }, o2.__Z7respondiiN6i3slib3vxl13Wasm_responseE = function() {
    return (o2.__Z7respondiiN6i3slib3vxl13Wasm_responseE = o2.asm._Z7respondiiN6i3slib3vxl13Wasm_responseE).apply(null, arguments);
  }, o2.__Z9add_layerRKNSt3__212basic_stringIcNS_11char_traitsIcEENS_9allocatorIcEEEEjjddddbi = function() {
    return (o2.__Z9add_layerRKNSt3__212basic_stringIcNS_11char_traitsIcEENS_9allocatorIcEEEEjjddddbi = o2.asm._Z9add_layerRKNSt3__212basic_stringIcNS_11char_traitsIcEENS_9allocatorIcEEEEjjddddbi).apply(null, arguments);
  }, o2.__Z12remove_layeri = function() {
    return (o2.__Z12remove_layeri = o2.asm._Z12remove_layeri).apply(null, arguments);
  }, o2.__Z21initialize_voxel_wasmiiiiiiiiddbbb = function() {
    return (o2.__Z21initialize_voxel_wasmiiiiiiiiddbbb = o2.asm._Z21initialize_voxel_wasmiiiiiiiiddbbb).apply(null, arguments);
  }, o2.__Z23uninitialize_voxel_wasmv = function() {
    return (o2.__Z23uninitialize_voxel_wasmv = o2.asm._Z23uninitialize_voxel_wasmv).apply(null, arguments);
  }, o2.__Z15set_view_matrixffffffffffffffff = function() {
    return (o2.__Z15set_view_matrixffffffffffffffff = o2.asm._Z15set_view_matrixffffffffffffffff).apply(null, arguments);
  }, o2.__Z21set_projection_matrixffffffffffffffff = function() {
    return (o2.__Z21set_projection_matrixffffffffffffffff = o2.asm._Z21set_projection_matrixffffffffffffffff).apply(null, arguments);
  }, o2.__Z12set_near_fardd = function() {
    return (o2.__Z12set_near_fardd = o2.asm._Z12set_near_fardd).apply(null, arguments);
  }, o2.__Z4cullv = function() {
    return (o2.__Z4cullv = o2.asm._Z4cullv).apply(null, arguments);
  }, o2.__Z31update_camera_pos_and_directiondddddd = function() {
    return (o2.__Z31update_camera_pos_and_directiondddddd = o2.asm._Z31update_camera_pos_and_directiondddddd).apply(null, arguments);
  }, o2.__Z8get_nearv = function() {
    return (o2.__Z8get_nearv = o2.asm._Z8get_nearv).apply(null, arguments);
  }, o2.__Z7get_farv = function() {
    return (o2.__Z7get_farv = o2.asm._Z7get_farv).apply(null, arguments);
  }, o2.__Z17begin_color_framebddd = function() {
    return (o2.__Z17begin_color_framebddd = o2.asm._Z17begin_color_framebddd).apply(null, arguments);
  }, o2.__Z11begin_framev = function() {
    return (o2.__Z11begin_framev = o2.asm._Z11begin_framev).apply(null, arguments);
  }, o2.__Z32get_texture_units_bound_in_framev = function() {
    return (o2.__Z32get_texture_units_bound_in_framev = o2.asm._Z32get_texture_units_bound_in_framev).apply(null, arguments);
  }, o2.__Z23get_active_texture_unitv = function() {
    return (o2.__Z23get_active_texture_unitv = o2.asm._Z23get_active_texture_unitv).apply(null, arguments);
  }, o2.__Z12set_viewportii = function() {
    return (o2.__Z12set_viewportii = o2.asm._Z12set_viewportii).apply(null, arguments);
  }, o2.__Z4drawv = function() {
    return (o2.__Z4drawv = o2.asm._Z4drawv).apply(null, arguments);
  }, o2.__Z11set_qualityi = function() {
    return (o2.__Z11set_qualityi = o2.asm._Z11set_qualityi).apply(null, arguments);
  }, o2.__Z10pick_depthii = function() {
    return (o2.__Z10pick_depthii = o2.asm._Z10pick_depthii).apply(null, arguments);
  }, o2.__Z11pick_objectii = function() {
    return (o2.__Z11pick_objectii = o2.asm._Z11pick_objectii).apply(null, arguments);
  }, o2.__Z11set_enabledib = function() {
    return (o2.__Z11set_enabledib = o2.asm._Z11set_enabledib).apply(null, arguments);
  }, o2.__Z23handle_masked_ui_updateijj = function() {
    return (o2.__Z23handle_masked_ui_updateijj = o2.asm._Z23handle_masked_ui_updateijj).apply(null, arguments);
  }, o2.__Z21set_scene_time_extentddb = function() {
    return (o2.__Z21set_scene_time_extentddb = o2.asm._Z21set_scene_time_extentddb).apply(null, arguments);
  }, o2.__Z20set_upscaling_limitsidd = function() {
    return (o2.__Z20set_upscaling_limitsidd = o2.asm._Z20set_upscaling_limitsidd).apply(null, arguments);
  }, o2.__Z21estimate_memory_usagei = function() {
    return (o2.__Z21estimate_memory_usagei = o2.asm._Z21estimate_memory_usagei).apply(null, arguments);
  }, o2.__Z16add_generic_meshjjjjjjjj = function() {
    return (o2.__Z16add_generic_meshjjjjjjjj = o2.asm._Z16add_generic_meshjjjjjjjj).apply(null, arguments);
  }, o2.__Z19remove_generic_meshi = function() {
    return (o2.__Z19remove_generic_meshi = o2.asm._Z19remove_generic_meshi).apply(null, arguments);
  }, o2.__Z30toggle_full_volume_extent_drawi = function() {
    return (o2.__Z30toggle_full_volume_extent_drawi = o2.asm._Z30toggle_full_volume_extent_drawi).apply(null, arguments);
  };
  var Eo = o2.___getTypeName = function() {
    return (Eo = o2.___getTypeName = o2.asm.__getTypeName).apply(null, arguments);
  };
  o2.___embind_register_native_and_builtin_types = function() {
    return (o2.___embind_register_native_and_builtin_types = o2.asm.__embind_register_native_and_builtin_types).apply(null, arguments);
  };
  var go, po = o2.___errno_location = function() {
    return (po = o2.___errno_location = o2.asm.__errno_location).apply(null, arguments);
  };
  function ho(e3) {
    function t2() {
      go || (go = true, o2.calledRun = true, C || (ne(), r2(o2), o2.onRuntimeInitialized && o2.onRuntimeInitialized(), oe()));
    }
    ue > 0 || (te(), ue > 0 || (o2.setStatus ? (o2.setStatus("Running..."), setTimeout(function() {
      setTimeout(function() {
        o2.setStatus("");
      }, 1), t2();
    }, 1)) : t2()));
  }
  if (o2.stackSave = function() {
    return (o2.stackSave = o2.asm.stackSave).apply(null, arguments);
  }, o2.stackRestore = function() {
    return (o2.stackRestore = o2.asm.stackRestore).apply(null, arguments);
  }, o2.stackAlloc = function() {
    return (o2.stackAlloc = o2.asm.stackAlloc).apply(null, arguments);
  }, o2.dynCall_iij = function() {
    return (o2.dynCall_iij = o2.asm.dynCall_iij).apply(null, arguments);
  }, o2.dynCall_jiji = function() {
    return (o2.dynCall_jiji = o2.asm.dynCall_jiji).apply(null, arguments);
  }, o2.dynCall_viijii = function() {
    return (o2.dynCall_viijii = o2.asm.dynCall_viijii).apply(null, arguments);
  }, o2.dynCall_iiiiij = function() {
    return (o2.dynCall_iiiiij = o2.asm.dynCall_iiiiij).apply(null, arguments);
  }, o2.dynCall_iiiiijj = function() {
    return (o2.dynCall_iiiiijj = o2.asm.dynCall_iiiiijj).apply(null, arguments);
  }, o2.dynCall_iiiiiijj = function() {
    return (o2.dynCall_iiiiiijj = o2.asm.dynCall_iiiiiijj).apply(null, arguments);
  }, o2.stringToUTF8 = R, o2.lengthBytesUTF8 = x, o2.addRunDependency = fe, o2.removeRunDependency = de, o2.FS_createPath = Oe.createPath, o2.FS_createDataFile = Oe.createDataFile, o2.FS_createPreloadedFile = Oe.createPreloadedFile, o2.FS_createLazyFile = Oe.createLazyFile, o2.FS_createDevice = Oe.createDevice, o2.FS_unlink = Oe.unlink, o2.addFunction = Q, le = function e3() {
    go || ho(), go || (le = e3);
  }, o2.run = ho, o2.preInit)
    for ("function" == typeof o2.preInit && (o2.preInit = [o2.preInit]); o2.preInit.length > 0; )
      o2.preInit.pop()();
  return ho(), e2.ready;
}, r.exports = n;
var a = e({ __proto__: null, default: o.exports }, [o.exports]);
export {
  a as v
};
//# sourceMappingURL=vxlLayer-RU7VDM4L.js.map
