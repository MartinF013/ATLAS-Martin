...
var p = class {
  constructor(t2) {
    this.handlers = /* @__PURE__ */ new Map(), this.counter = 0, this.handlerCounts = /* @__PURE__ */ new Map(), this.view = t2, this.inputManager = null;
  }
  connect(t2) {
    t2 && this.disconnect(), this.inputManager = t2, this.handlers.forEach(({ handler: t3, priority: e }, a) => this.inputManager.installHandlers(a, [t3], e));
  }
  disconnect() {
    this.inputManager && this.handlers.forEach((t2, e) => this.inputManager.uninstallHandlers(e)), this.inputManager = null;
  }
  destroy() {
  ...
  on(t2, e, a, r2) {
    ...
    let p2, l;
    Array.isArray(e) ? l = e : (p2 = e, l = []), "function" == typeof a ? p2 = a : r2 = a, r2 = null != r2 ? r2 : g.DEFAULT;
    const u = this._createUniqueGroupName(), m2 = new c2(this.view, i3, l, p2);
    this.handlers.set(u, { handler: m2, priority: r2 });
    for (const n of i3) {
      const t3 = this.handlerCounts.get(n) || 0;
      this.handlerCounts.set(n, t3 + 1);
    }
    return this.inputManager && this.inputManager.installHandlers(u, [m2], r2), { remove: () => this._removeHandler(u, i3) };
  }
  hasHandler(t2) {
    return !!this.handlerCounts.get(t2);
  }
  _removeHandler(t2, e) {
    if (this.handlers.has(t2)) {
      this.handlers.delete(t2);
      for (const t3 of e) {
        const e2 = this.handlerCounts.get(t3);
        void 0 === e2 ? console.error("Trying to remove handler for event that has no handlers registered: ", t3) : 1 === e2 ? this.handlerCounts.delete(t3) : this.handlerCounts.set(t3, e2 - 1);
      }
    }
...
var c2 = class extends i {
  ...
  _wrapDrag(e) {
    ...
    if ("start" === i3 && (this.latestDragStart = a), !this.latestDragStart)
      return;
    const p2 = a.pointer.native, c3 = a.buttons, { cancelable: l, timestamp: u } = e, m2 = { x: this.latestDragStart.center.x, y: this.latestDragStart.center.y };
    return "end" === i3 && (this.latestDragStart = void 0), { type: "drag", action: i3, x: n, y: r2, origin: m2, pointerType: o2, button: s2, buttons: c3, radius: a.radius, angle: m(a.angle), native: p2, timestamp: u, cancelable: l, stopPropagation: () => e.stopPropagation(), async: (t2) => e.async(t2), preventDefault: () => e.preventDefault() };
  }
  _wrapKeyDown(t2) {
...
//# sourceMappingURL=chunk-WTYOB7O7.js.map
