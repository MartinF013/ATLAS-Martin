...
var Panel = proxyCustomElement(class extends H {
  ...
  renderBackButton() {
    ...
    return showBackButton ? h("calcite-action", { "aria-label": label, class: CSS.backButton, icon, key: "back-button", onClick: backButtonClick, ref: this.setBackRef, scale: "s", slot: SLOTS2.headerActionsStart, text: label }) : null;
  }
  renderHeaderContent() {
  ...
  renderHeaderSlottedContent() {
    return h("div", { class: CSS.headerContent, key: "slotted-header-content" }, h("slot", { name: SLOTS2.headerContent }));
  }
  renderHeaderStartActions() {
    const { el } = this;
    const hasStartActions = getSlotted(el, SLOTS2.headerActionsStart);
    return hasStartActions ? h("div", { class: { [CSS.headerActionsStart]: true, [CSS.headerActions]: true }, key: "header-actions-start" }, h("slot", { name: SLOTS2.headerActionsStart })) : null;
  }
  renderHeaderActionsEnd() {
    const { dismiss, dismissible, el, intlClose } = this;
    const text = intlClose || TEXT.close;
    const dismissibleNode = dismissible ? h("calcite-action", { "aria-label": text, icon: ICONS.close, onClick: dismiss, ref: this.setDismissRef, text }) : null;
    const slotNode = h("slot", { name: SLOTS2.headerActionsEnd });
    const hasEndActions = getSlotted(el, SLOTS2.headerActionsEnd);
  ...
  renderMenu() {
    ...
    const hasMenuItems = getSlotted(el, SLOTS2.headerMenuActions);
    return hasMenuItems ? h("calcite-action-menu", { flipPlacements: ["top", "bottom"], key: "menu", label: intlOptions || TEXT.options, open: menuOpen, placement: "bottom-end" }, h("calcite-action", { icon: ICONS.menu, slot: SLOTS.trigger, text: intlOptions || TEXT.options }), h("slot", { name: SLOTS2.headerMenuActions })) : null;
  }
  renderHeaderNode() {
    const { el, showBackButton } = this;
    const backButtonNode = this.renderBackButton();
    const hasHeaderSlottedContent = getSlotted(el, SLOTS2.headerContent);
    const headerContentNode = hasHeaderSlottedContent ? this.renderHeaderSlottedContent() : this.renderHeaderContent();
    ...
    return actionsNodeStart || headerContentNode || actionsNodeEnd || headerMenuNode || showBackButton ? h("header", { class: CSS.header }, backButtonNode, actionsNodeStart, headerContentNode, actionsNodeEnd, headerMenuNode) : null;
  }
  renderFooterNode() {
    const { el } = this;
    const hasFooterSlottedContent = getSlotted(el, SLOTS2.footer);
    const hasFooterActions = getSlotted(el, SLOTS2.footerActions);
    return hasFooterSlottedContent || hasFooterActions ? h("footer", { class: CSS.footer, key: "footer" }, hasFooterSlottedContent ? h("slot", { key: "footer-slot", name: SLOTS2.footer }) : null, hasFooterActions ? h("slot", { key: "footer-actions-slot", name: SLOTS2.footerActions }) : null) : null;
  }
  renderContent() {
    const { el } = this;
    const hasFab = getSlotted(el, SLOTS2.fab);
  ...
  renderFab() {
    return h("div", { class: CSS.fabContainer }, h("slot", { name: SLOTS2.fab }));
  }
  render() {
    const { dismissed, dismissible, loading, panelKeyDownHandler } = this;
    const panelNode = h("article", { "aria-busy": toAriaBoolean(loading), class: CSS.container, hidden: dismissible && dismissed, onKeyDown: panelKeyDownHandler, ref: this.setContainerRef, tabIndex: dismissible ? 0 : -1 }, this.renderHeaderNode(), this.renderContent(), this.renderFooterNode());
...
@esri/calcite-components/dist/components/calcite-panel.js:
   ...
   *)
*/
//# sourceMappingURL=calcite-panel-CKH4JSDF.js.map
