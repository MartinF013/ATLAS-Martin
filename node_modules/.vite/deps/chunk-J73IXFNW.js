...
function n(n2, f, i) {
  ...
  const a2 = (i == null ? void 0 : i.minReduction) ?? 0, h = (i == null ? void 0 : i.originalIndices) || null, g = h ? h.length : 0, y = (i == null ? void 0 : i.componentOffsets) || null;
  let U = 0;
  if (y)
  ...
  (null == o || o.length < 2 * w) && (o = new Uint32Array(a(2 * w)));
  for (let t = 0; t < 2 * w; t++)
    o[t] = 0;
  let A = 0;
  const m = !!y && !!h, b = m ? g : u, d = m ? new Uint32Array(g) : null, p = 1.96;
  let M = 0 !== a2 ? Math.ceil(4 * p * p / (a2 * a2) * a2 * (1 - a2)) : b, q = 1, j = y ? y[1] : b;
  for (let t = 0; t < b; t++) {
    if (t === M) {
      const n4 = 1 - A / t;
      if (n4 + p * Math.sqrt(n4 * (1 - n4) / t) < a2)
        return null;
      M *= 2;
    }
    if (t === j) {
...
//# sourceMappingURL=chunk-J73IXFNW.js.map
